...
void sql_expr_attach_subtrees(sql_expr_t *root, sql_expr_t *left,
                              sql_expr_t *right) {
  if (root == NULL) {
    sql_expr_free(left);
    sql_expr_free(right);
 ...
 */
gboolean sql_expr_is_dotted_name(const sql_expr_t *p, const char *prefix,
                                 const char *suffix) {
  if (p && p->op == TK_DOT) {
    return sql_expr_is_id(p->left, prefix) && sql_expr_is_id(p->right, suffix);
  }
  return FALSE;
}

void sql_expr_get_dotted_names(const sql_expr_t *p, char *db, int db_len,
                               char *table, int tb_len, char *col,
                               int col_len) {
  if (p && p->op == TK_DOT) {
...
sql_expr_t *sql_expr_list_find_fullname(sql_expr_list_t *list,
...
sql_column_t *sql_column_new() { return g_new0(struct sql_column_t, 1); }
...
sql_column_list_t *sql_column_list_append(sql_column_list_t *list,
                                          sql_column_t *col) {
  if (col == NULL)
    return list;
  if (list == NULL) {
...
void sql_drop_database_free(sql_drop_database_t *p) {
  if (!p)
    return;
  if (p && p->schema_name) {
...
sql_src_list_t *sql_src_list_append(sql_src_list_t *p, sql_token_t *tname,
                                    ...
                                    sql_expr_t *on_clause,
                                    sql_id_list_t *using_clause) {
...
sql_index_hint_t *sql_index_hint_new() { return g_new0(sql_index_hint_t, 1); }

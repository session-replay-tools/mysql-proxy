...
static gboolean sql_variable_is_silent_value(struct sql_variable_t *p,
                                             const char *value) {
  GList *l;
...
static gboolean sql_variable_is_allowed_value(struct sql_variable_t *p,
                                              const char *value) {
  GList *l;
...
gboolean sql_filter_vars_load_str_rules(const char *json_str) {
  if (!cetus_variables) {
    cetus_variables = g_hash_table_new_full(str_case_hash, str_case_equal, NULL,
                                            (GDestroyNotify)sql_variable_free);
  }
    ...
    if (silent_array) {
      ...
      for (; silent; silent = silent->next) {
        if (silent->valuestring) {
          ...
          var->silent_values =
              g_list_append(var->silent_values, g_strdup(silent->valuestring));
        }
      }
    }
...
void sql_filter_vars_load_default_rules() {
  static const char *default_var_rule =
      ...
      "      \"type\": \"string\","
      "      \"allowed_values\": "
      "[\"latin1\",\"ascii\",\"gb2312\",\"gbk\",\"utf8\",\"utf8mb4\","
      "\"binary\",\"big5\"]"
      "    },"
      "    {"

...
enum sql_join_type_t {
  JT_LEFT = 0x01,  /* Left outer join */
  JT_ERROR = 0x02, /* unknown or unsupported join type */
};
...
struct sql_expr_t {
  ...
  enum sql_var_scope_t
      var_scope;     /* variable scope: SESSION(default) or GLOBAL */
...
#define sql_expr_id(A) (A ? ((A->op == TK_ID) ? A->token_text : NULL) : NULL)
...
void sql_expr_get_dotted_names(const sql_expr_t *p, char *db, int db_len,
                               char *table, int tb_len, char *col, int col_len);

...
static int string_to_code(const char *str) {
  struct code_map_t {
    ...
    int code;
      ...
      {"READWRITE", MODE_READWRITE},    {"READONLY", MODE_READONLY},
      {"SCOPE_LOCAL", P_SCOPE_LOCAL},   {"SCOPE_GLOBAL", P_SCOPE_GLOBAL},
  };
  int i;
...
static gboolean parser_find_key(sql_property_parser_t *parser,
  ...
  static const struct property_desc_t {
    ...
    value_parse_func get_value;
      ...
      {"scope", offsetof(struct sql_property_t, scope), TYPE_INT,
       string_to_code},
      {"transaction", offsetof(struct sql_property_t, transaction), TYPE_INT,
       string_to_code},
...
static gboolean parser_set_value(sql_property_parser_t *parser, void *object,
                                 const char *token, int len) {

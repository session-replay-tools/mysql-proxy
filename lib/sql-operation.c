...
static gboolean string_array_contains(const char **sa, int size,
                                      const char *str) {
  int i = 0;
  for (; i < size; ++i) {
...
void sql_set_names(sql_context_t *ps, char *val) {
  ...
  const char *charsets[] = {"latin1", "ascii",   "gb2312", "gbk",
                            "utf8",   "utf8mb4", "big5"};
...
void sql_set_transaction(sql_context_t *ps, int scope, int rw_feature,
                         int level) {
  if (ps->property) {
    sql_context_set_error(ps, PARSE_NOT_SUPPORT,
                          "Commanding comment is not allowed in SET clause");

...
void g_fast_stream_hexdump(const char *msg, const void *_s, size_t len) {
  GString *hex;
  size_t i;
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_timeout)
{
  ...
  g_debug("%s, con:%p:call proxy_timeout, idle timeout:%d, diff:%d", G_STRLOC,
          con, idle_timeout, diff);
    ...
    if (con->server && !con->client->is_server_conn_reserved) {
      ...
      if (!con->resultset_is_needed && con->candidate_fast_streamed) {
        g_critical("%s, fast_stream_last_exec_index:%d, need more:%d", G_STRLOC,
                   con->fast_stream_last_exec_index,
                   con->fast_stream_need_more);
        g_critical("%s, eof_last_met:%d, eof_met_cnt:%d", G_STRLOC,
                   con->eof_last_met, con->eof_met_cnt);
        g_critical("%s, partically_record_left_cnt:%d, analysis_next_pos:%d, "
                   "cur_resp_len:%d",
                   G_STRLOC, con->partically_record_left_cnt,
                   (int)con->analysis_next_pos, (int)con->cur_resp_len);
        g_critical("%s, last_payload_len:%d, last_record_payload_len:%d",
                   G_STRLOC, con->last_payload_len,
                   con->last_record_payload_len);
        g_fast_stream_hexdump(G_STRLOC, con->record_last_payload,
                              con->last_record_payload_len);
      }
      g_critical("%s, con:%p read query result timeout, sql:%s", G_STRLOC, con,
                 con->orig_sql->str);
...
static int store_server_ndx_in_prepared_resp(proxy_resultset_t *res,
                                             int index) {
...
proxy_c_read_query_result(network_mysqld_con *con) {
  ...
  case INJ_ID_CHANGE_USER:
    if (con->is_changed_user_failed) {
      ...
      network_mysqld_con_send_error_full(
          con->client, C("Access denied for serving requests"),
          ER_ACCESS_DENIED_ERROR, "29001");
  ...
  if (is_continue) {
      ...
      if (inj->id != INJ_ID_COM_STMT_PREPARE) {
        if (res->qstat.server_status & SERVER_STATUS_IN_TRANS) {
          if (recv_sock->is_read_only) {
            g_message("%s: SERVER_STATUS_IN_TRANS true from read server",
                      G_STRLOC);
          } else {
    ...
    g_debug("%s: con multiple_server_mode:%d", G_STRLOC,
            con->multiple_server_mode);
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_auth) { return do_read_auth(con); }
...
static int network_mysqld_con_handle_insert_id_response(network_mysqld_con *con,
  ...
  rows = g_ptr_array_new_with_free_func(
...
static int process_other_set_command(network_mysqld_con *con, const char *key,
                                     const char *s,
...
static int process_set_names(network_mysqld_con *con, char *s,
                             mysqld_query_attr_t *query_attr) {
  network_socket *sock = con->client;
  size_t s_len = strlen(s);
  ...
  if (s && strcasecmp(s, con->srv->default_charset) != 0 &&
      sock->charset_code == DEFAULT_CHARSET) {
    g_warning("%s: charset code:%d, charset:%s", G_STRLOC, sock->charset_code,
              s == NULL ? "NULL" : s);
  }
  return 0;
}
...
static int process_trans_query(network_mysqld_con *con) {
  proxy_plugin_con_t *st = con->plugin_con_state;
  sql_context_t *context = st->sql_context;
  g_debug("%s: visit process_trans_query here:%d", G_STRLOC,
          context->stmt_type);
    ...
    if (sql_context_is_autocommit_off(context)) {
      ...
      if (con->is_in_transaction) {
        ...
        if (con->multiple_server_mode) {
          int type = BACKEND_TYPE_RW;
          if (!proxy_get_backend_ndx(con, type, FALSE)) {
            g_critical("%s:serious error when change from slave to master",
                       G_STRLOC);
            /* no master connection */
            return PROXY_NO_CONNECTION;
          }
          g_debug("%s: send autocommit on to master for con:%p", G_STRLOC, con);
        }
      }
...
static int process_filter_for_trans_query(network_mysqld_con *con,
...
static int process_non_trans_query(network_mysqld_con *con,
  ...
  case STMT_SELECT: {
    ...
    if (is_insert_id == TRUE) {
      ...
      network_mysqld_con_handle_insert_id_response(con, last_insert_id_name,
                                                   con->last_insert_id);
      return PROXY_SEND_RESULT;
    }
      ...
      if (expr && expr->op == TK_EQ) {
        ...
        if (sql_context_is_autocommit_off(context)) {
          ...
          if (left->op == TK_ID && right->token_text) {
            process_other_set_command(con, left->token_text, right->token_text,
                                      query_attr);
          }
        }
      }
  ...
  default:
    if (con->is_auto_commit) {
      if (context->stmt_type == STMT_USE) {
        ...
        g_debug("%s:set default db:%s for con:%p", G_STRLOC,
...
static void proxy_inject_packet(network_mysqld_con *con, int type,
                                int resp_type, GString *payload,
                                gboolean resultset_is_needed,
                                gboolean is_fast_streamed) {
  proxy_plugin_con_t *st = con->plugin_con_state;
...
static int change_stmt_id(network_mysqld_con *con, uint32_t stmt_id) {
  ...
  if (con->servers != NULL) {
    if (index >= con->servers->len) {
      g_warning("%s:index:%d, stmt id:%d is too big, servers len:%d", G_STRLOC,
                index, (int)stmt_id, con->servers->len);
      return -1;
    }
    ...
    g_debug("change conn:%p, server:%p stmt_id:%d, fd:%d, new back ndx:%d", con,
            con->server, (int)stmt_id, con->server->fd, st->backend_ndx);
...
static int change_server_by_rw(network_mysqld_con *con, int backend_ndx) {
  if (backend_ndx >= 0) {
    ...
    if (con->servers != NULL) {
      ...
      g_debug("conn:%p, change_server_by_rw,ndx:%d, index:%d, st ndx:%d", con,
              backend_ndx, index, st->backend_ndx);
      con->server = g_ptr_array_index(con->servers, index);
...
static int adjust_sql_mode(network_mysqld_con *con,
                           mysqld_query_attr_t *query_attr) {
    ...
    if (strcasecmp(clt_sql_mode, srv_sql_mode) != 0) {
      if (strcmp(clt_sql_mode, "") != 0) {
        ...
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                            INJ_ID_CHANGE_SQL_MODE, packet, TRUE, FALSE);
      } else {
        GString *packet = g_string_new(NULL);
        g_string_append_c(packet, (char)COM_QUERY);
        g_string_append(packet, "SET sql_mode=''");
      ...
      g_string_assign_len(con->server->sql_mode, con->client->sql_mode->str,
                          con->client->sql_mode->len);
    }
  } else {
    g_string_assign_len(con->server->sql_mode, con->client->sql_mode->str,
...
static int adjust_charset(network_mysqld_con *con,
                          mysqld_query_attr_t *query_attr) {
  ...
  if (!query_attr->charset_set) {
    if (!g_string_equal(con->client->charset, con->server->charset)) {
      ...
      if (charset->len > 0) {
        ...
        g_string_assign_len(con->server->charset_client, charset->str,
                            charset->len);
        g_string_assign_len(con->server->charset_connection, charset->str,
                            charset->len);
        g_string_assign_len(con->server->charset_results, charset->str,
                            charset->len);
      }
  ...
  if (con->srv->charset_check) {
    if (strcmp(con->client->charset->str, con->srv->default_charset) != 0) {
      g_message("%s: client charset:%s, default charset:%s, client address:%s",
                G_STRLOC, con->client->charset->str, con->srv->default_charset,
                con->client->src->name->str);
    }
  }

  if (!query_attr->charset_client_set) {
    if (!g_string_equal(con->client->charset_client,
                        con->server->charset_client)) {
      if (con->client->charset_client->len > 0) {
        ...
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND, INJ_ID_CHAR_SET_CLT,
                            packet, TRUE, FALSE);
      }
      GString *charset_client = con->client->charset_client;
      g_string_assign_len(con->server->charset_client, charset_client->str,
                          charset_client->len);
    }
  } else {
    GString *charset_client = con->client->charset_client;
  ...
  if (!query_attr->charset_connection_set) {
    if (!g_string_equal(con->client->charset_connection,
      ...
      if (con->client->charset_connection->len > 0) {
        ...
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND, INJ_ID_CHAR_SET_CONN,
                            packet, TRUE, FALSE);
      }
      GString *charset_conn = con->client->charset_connection;
      g_string_assign_len(con->server->charset_connection, charset_conn->str,
                          charset_conn->len);
    }
  } else {
    GString *charset_conn = con->client->charset_connection;
  ...
  if (!query_attr->charset_results_set) {
    if (!g_string_equal(con->client->charset_results,
                        con->server->charset_results)) {
      if (con->client->charset_results->len > 0) {
        ...
        proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND,
                            INJ_ID_CHAR_SET_RESULTS, packet, TRUE, FALSE);
      } else {
        GString *packet = g_string_new(NULL);
        g_string_append_c(packet, (char)COM_QUERY);
      ...
      g_string_assign_len(con->server->charset_results, charset_results->str,
                          charset_results->len);
    }
  } else {
    GString *charset_results = con->client->charset_results;
  ...
  if (query_attr->charset_reset) {
    ...
    if (strcmp(con->client->charset->str, "") == 0) {
      g_warning("%s: client charset is empty:%s", G_STRLOC,
                con->client->src->name->str);
      g_string_append(packet, "''");
    } else {
    ...
    proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND, INJ_ID_SET_NAMES, packet,
                        TRUE, FALSE);
  }
...
static int adjust_default_db(network_mysqld_con *con,
                             enum enum_server_command cmd) {
  GString *clt_default_db = con->client->default_db;
  GString *srv_default_db = con->server->default_db;

  g_debug(G_STRLOC " default client db:%s",
          clt_default_db ? clt_default_db->str : "null");
  g_debug(G_STRLOC " default server db:%s",
          srv_default_db ? srv_default_db->str : "null");
  ...
  if (clt_default_db->len > 0) {
    if (!g_string_equal(clt_default_db, srv_default_db)) {
      ...
      proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND, INJ_ID_CHANGE_DB,
                          packet, TRUE, FALSE);
...
static int adjust_user(network_mysqld_con *con) {
  g_debug("%s: user:%s try to robs conn from user:%s, server:%p for con:%p",
          G_STRLOC, con->client->response->username->str,
          con->server->response->username->str, con->server, con);

  GString *hashed_password = g_string_new(NULL);
  const char *user = con->client->response->username->str;
  cetus_users_get_hashed_server_pwd(con->srv->priv->users, user,
                                    hashed_password);
  if (hashed_password->len == 0) {
    ...
    g_debug("%s: charset:%d when change user", G_STRLOC,
            con->client->charset_code);
...
static int adjust_multi_stmt(network_mysqld_con *con,
                             enum enum_server_command cmd) {
  if (con->client->is_multi_stmt_set != con->server->is_multi_stmt_set) {
    ...
    g_string_append_c(packet, (char)0);
    proxy_inject_packet(con, PROXY_QUEUE_ADD_PREPEND, INJ_ID_CHANGE_MULTI_STMT,
                        packet, TRUE, FALSE);
...
static int proxy_handle_local_query(network_mysqld_con *con,
                                    sql_context_t *context) {
...
static int process_quit_cmd(network_mysqld_con *con, int backend_ndx,
                            int *disp_flag) {
  if (backend_ndx < 0 || (!con->is_in_transaction &&
...
static int forced_visit(network_mysqld_con *con, proxy_plugin_con_t *st,
  ...
  int type =
      (context->rw_flag & CF_FORCE_MASTER) ? BACKEND_TYPE_RW : BACKEND_TYPE_RO;
  ...
  if (st->backend == NULL || (st->backend && st->backend->type != type)) {
    gboolean success =
        proxy_get_backend_ndx(con, type, context->rw_flag & CF_FORCE_SLAVE);
    if (!success) {
      if (type == BACKEND_TYPE_RO) {
        con->slave_conn_shortaged = 1;
...
static int process_rw_split(network_mysqld_con *con, proxy_plugin_con_t *st,
                            sql_context_t *context,
                            mysqld_query_attr_t *query_attr,
                            int is_under_sess_scope, int command,
                            int *disp_flag) {
...
static int process_query_or_stmt_prepare(network_mysqld_con *con,
  ...
  g_debug("%s process query:%s for con:%p", G_STRLOC, con->orig_sql->str, con);

  if (context->rc == PARSE_SYNTAX_ERR) {
    char *msg = context->message;
    g_message("%s SQL syntax error: %s. while parsing: %s", G_STRLOC, msg,
              con->orig_sql->str);
  ...
  if ((context->rw_flag & CF_FORCE_SLAVE) &&
      (((!con->srv->check_sql_loosely) && (context->rw_flag & CF_WRITE)) ||
    ...
    if (con->is_in_transaction) {
      network_mysqld_con_send_error(con->client,
                                    C("Force transaction on read-only slave"));
    } else {
      network_mysqld_con_send_error(con->client,
                                    C("Force write on read-only slave"));
    }
    *disp_flag = PROXY_SEND_RESULT;
    return 0;
  }
  ...
  if (context->rw_flag & (CF_FORCE_MASTER | CF_FORCE_SLAVE)) {
    ...
    if (context->stmt_type == STMT_SET_TRANSACTION) {
      ...
      if (feat->scope == SCOPE_SESSION) {
        g_message("%s:set session transaction sql:%s for con:%p", G_STRLOC,
                  con->orig_sql->str, con);
        if (feat->rw_feature) {
          st->trx_read_write = feat->rw_feature;
        } else if (feat->level) {
          st->trx_isolation_level = feat->level;
        } else {
          g_warning("%s:unexpected transaction feature:%s", G_STRLOC,
                    con->orig_sql->str);
        }
      }
    }
...
static network_mysqld_stmt_ret network_read_query(network_mysqld_con *con,
  ...
  g_debug("%s: command:%d, backend ndx:%d, con:%p, orig sql:%s", G_STRLOC,
          command, backend_ndx, con, con->orig_sql->str);
  ...
  case COM_QUERY:
    if (!process_query_or_stmt_prepare(con, st, &packet, &query_attr, command,
                                       &disp_flag)) {
      return disp_flag;
    }
  ...
  case COM_CHANGE_USER:
    network_mysqld_con_send_error(con->client,
                                  C("(proxy) unable to process change user"));
    return PROXY_SEND_RESULT;
  default:
    break;
  } /* end switch */
  ...
  if (con->server == NULL) {
    ...
    if (!con->is_prepared && st->backend &&
        st->backend->type == BACKEND_TYPE_RO) {
      if (st->backend->state != BACKEND_STATE_UP &&
          st->backend->state != BACKEND_STATE_UNKNOWN) {
        last_resort = TRUE;
      }
  ...
  if (con->is_in_sess_context) {
    ...
    g_debug("%s: set is_in_sess_context true for con server:%p", G_STRLOC,
            con->server);
  } else {
    con->server->is_in_sess_context = 0;
    g_debug("%s: set is_in_sess_context false for con server:%p", G_STRLOC,
            con->server);
  }
  ...
  if (backend->state != BACKEND_STATE_UP &&
    ...
    case COM_QUERY: {
      network_mysqld_con_send_error_full(con->client,
                                         ...
                                         ER_ABORTING_CONNECTION, "29001");
      g_message(G_STRLOC
                ": ER_ABORTING_CONNECTION, proxy stops serving requests");
      return PROXY_SEND_RESULT;
    }
    default:
      break;
  ...
  case COM_QUERY:
    if (context->stmt_type == STMT_SELECT && con->is_read_ro_server_allowed) {
      if (con->srv->is_fast_stream_enabled) {
        if ((!con->srv->sql_mgr) ||
            (con->srv->sql_mgr->sql_log_switch != ON &&
             con->srv->sql_mgr->sql_log_switch != REALTIME)) {
          proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND, INJ_ID_COM_QUERY,
                              payload, FALSE, TRUE);
        } else {
          proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND, INJ_ID_COM_QUERY,
                              payload, FALSE, FALSE);
    ...
    break;
  case COM_STMT_PREPARE:
    proxy_inject_packet(con, PROXY_QUEUE_ADD_APPEND, INJ_ID_COM_STMT_PREPARE,
                        payload, TRUE, FALSE);
    break;
  default:
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_query)
{
  ...
  if (con->server != NULL) {
    if (con->last_backend_type != st->backend->type) {
      g_message("%s server_attr_changed, last backend type:%d, now type:%d",
  ...
  if (server_attr_changed && con->client->is_server_conn_reserved) {
    g_message("%s server attr changed and conn_reserved true, stop process",
              G_STRLOC);
    network_mysqld_con_send_error(
        con->client, C("(proxy) unable to continue processing command"));
  ...
  if (ret != PROXY_SEND_NONE && ret != PROXY_SEND_RESULT) {
    if (con->server == NULL || ret == PROXY_NO_CONNECTION) {
      g_debug("%s: I have no server backend, con:%p for user:%s", G_STRLOC, con,
              con->client->response->username->str);
  ...
  case PROXY_NO_DECISION:
    if (st->injected.queries->length) {
      g_critical("%s: discarding %d elements from the queue.", G_STRLOC,
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_read_query_result)
{
  ...
  /* TODO if attribute adjustment fails, then the backend connection should
   * not be put to pool */
  switch (con->parse.command) {
  case COM_QUERY:
  ...
  case COM_STMT_EXECUTE: {
    ...
    if (query && query->query_status == MYSQLD_PACKET_ERR) {
      ...
      if (!network_mysqld_proto_get_err_packet(&packet, err_packet)) {
        /* special processing for mgr paritition errors */
        if (strcasecmp(err_packet->errmsg->str,
                       "Error on observer while running replication hook "
                       "'before_commit'.") == 0) {
          g_warning("network partition is possibly happened, check "
                    "if transaction is "
                    "committed for server:%s,sql:%s",
                    con->server->dst->name->str, con->orig_sql->str);
        }
      } else {
        g_message("%s:dst:%s,sql:%s", G_STRLOC, con->server->dst->name->str,
                  con->orig_sql->str);
      }
    ...
    case COM_STMT_EXECUTE: {
      ...
      if (!com_query->was_resultset) {
        ...
        if (inj->qstat.insert_id > 0) {
          ...
          g_debug("%s: last insert id:%d for con:%p", G_STRLOC,
                  (int)con->last_insert_id, con);
        }
      }
      ...
      g_debug("%s: server status, got: %d, con:%p", G_STRLOC,
  ...
  if (send_sock->send_queue->chunks) {
    g_debug("%s: send queue is not empty:%d", G_STRLOC,
            send_sock->send_queue->chunks->length);
    con->state = ST_SEND_QUERY_RESULT;
  } else {
    /*
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_init)
{
  ...
  proxy_plugin_con_t *st = proxy_plugin_con_new();
  ...
  if (config->connect_timeout_dbl >= 0) {
    chassis_timeval_from_double(&con->connect_timeout,
                                config->connect_timeout_dbl);
  }
...
proxy_c_disconnect_client(network_mysqld_con *con) {
  ...
  if (client_abnormal_close) {
    ...
    if (con->is_in_transaction || !con->is_auto_commit ||
        con->is_in_sess_context) {
      con->server_to_be_closed = 1;
    } else {
...
static void proxy_plugin_con_free(network_mysqld_con *con,
                                  proxy_plugin_con_t *st) {
...
NETWORK_MYSQLD_PLUGIN_PROTO(proxy_disconnect_client)
{
  ...
  if (con->server && !con->server_to_be_closed) {
    if (con->state == ST_CLOSE_CLIENT || con->prev_state <= ST_READ_QUERY) {
      ...
      if (network_pool_add_conn(con, 0) != 0) {
        g_message("%s, con:%p:server conn returned to pool failed", G_STRLOC,
                  con);
      }
    }
  }
...
void network_mysqld_proxy_plugin_free(chassis *chas,
                                      chassis_plugin_config *config) {
...
network_mysqld_proxy_plugin_get_options(chassis_plugin_config *config) {
  ...
  chassis_options_add(
      &opts, "proxy-address", 'P', 0, OPTION_ARG_STRING, &(config->address),
      "listening address:port of the proxy-server (default: :4040)",
      "<host:port>", NULL, show_proxy_address,
      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(
      &opts, "proxy-read-only-backend-addresses", 'r', 0,
      OPTION_ARG_STRING_ARRAY, &(config->read_only_backend_addresses),
      "address:port of the remote slave-server (default: not set)",
      "<host:port>", NULL, show_proxy_read_only_backend_address,
      SAVE_OPTS_PROPERTY);

  chassis_options_add(
      &opts, "proxy-backend-addresses", 'b', 0, OPTION_ARG_STRING_ARRAY,
      &(config->backend_addresses),
      "address:port of the remote backend-servers (default: 127.0.0.1:3306)",
      "<host:port>", NULL, show_proxy_backend_addresses, SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "proxy-connect-timeout", 0, 0, OPTION_ARG_DOUBLE,
                      &(config->connect_timeout_dbl),
                      "connect timeout in seconds (default: 2.0 seconds)", NULL,
                      assign_proxy_connect_timeout, show_proxy_connect_timeout,
                      ALL_OPTS_PROPERTY);

  chassis_options_add(&opts, "proxy-read-timeout", 0, 0, OPTION_ARG_DOUBLE,
                      &(config->read_timeout_dbl),
                      "read timeout in seconds (default: 600 seconds)", NULL,
                      assign_proxy_read_timeout, show_proxy_read_timeout,
                      ALL_OPTS_PROPERTY);

  chassis_options_add(&opts, "proxy-write-timeout", 0, 0, OPTION_ARG_DOUBLE,
                      &(config->write_timeout_dbl),
                      "write timeout in seconds (default: 600 seconds)", NULL,
                      assign_proxy_write_timeout, show_proxy_write_timeout,
                      ALL_OPTS_PROPERTY);

  chassis_options_add(&opts, "proxy-allow-ip", 0, 0, OPTION_ARG_STRING,
                      &(config->allow_ip), "allow user@IP for proxy permission",
                      NULL, NULL, show_proxy_allow_ip, SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "proxy-deny-ip", 0, 0, OPTION_ARG_STRING,
                      &(config->deny_ip), "deny user@IP for proxy permission",
                      NULL, NULL, show_proxy_deny_ip, SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "read-master-percentage", 0, 0, OPTION_ARG_INT,
                      &(config->read_master_percentage), "range [0, 100]", NULL,
                      assign_read_master_percentage,
                      show_read_master_percentage, ALL_OPTS_PROPERTY);
...
int network_mysqld_proxy_plugin_apply_config(chassis *chas,
  ...
  if (network_backends_load_config(g->backends, chas) != -1) {
    ...
    evtimer_set(&chas->auto_create_conns_event, check_and_create_conns_func,
                chas);
  ...
  chassis_config_register_service(chas->config_manager, config->address,
                                  "proxy");

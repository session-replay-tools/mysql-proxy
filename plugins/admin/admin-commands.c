...
void admin_select_all_backends(network_mysqld_con* con)
{
  ...
  int i;
  char buffer[32];
  static char *states[] = {"unknown",     "up",      "down",
                           "maintaining", "deleted", "offline"};
...
void admin_select_conn_details(network_mysqld_con *con)
{
  ...
  GHashTable *back_user_conn_hash_table =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_table_free_all);
  ...
  for (i = 0; i < len; i++) {
    ...
    g_hash_table_insert(back_user_conn_hash_table,
                        g_strdup(backend->addr->name->str), table);
  }
  ...
  for (i = 0; i < len; i++) {
    ...
    if (con->servers != NULL) {
      for (j = 0; j < con->servers->len; j++) {
        ...
        used_conns_t *total_used =
            g_hash_table_lookup(table, con->client->response->username->str);
        if (total_used == NULL) {
          total_used = g_new0(used_conns_t, 1);
          g_hash_table_insert(table,
                              g_strdup(con->client->response->username->str),
                              total_used);
        }
        total_used->num++;
      }
      ...
      if (total_used == NULL) {
        total_used = g_new0(used_conns_t, 1);
        g_hash_table_insert(
            table, g_strdup(con->client->response->username->str), total_used);
      }
...
#define MAKE_FIELD_DEF_4_COL(fields, col1_name, col2_name, col3_name,          \
                             col4_name)                                        \
  do {                                                                         \
    MYSQL_FIELD *field = network_mysqld_proto_fielddef_new();                  \
    field->name = col1_name;                                                   \
...
void admin_acl_add_rule(network_mysqld_con *con, gboolean is_white, char *addr)
{
  ...
  chassis *chas = con->srv;
  int affected = cetus_acl_add_rule_str(
      chas->priv->acl, is_white ? ACL_WHITELIST : ACL_BLACKLIST, addr);
  if (chas->config_manager->type == CHASSIS_CONF_MYSQL) {
    network_mysqld_con_send_ok_full(con->client, affected, 0,
...
void admin_acl_delete_rule(network_mysqld_con *con, gboolean is_white, char *addr)
{
  ...
  chassis *chas = con->srv;
  int affected = cetus_acl_delete_rule_str(
      chas->priv->acl, is_white ? ACL_WHITELIST : ACL_BLACKLIST, addr);
  if (chas->config_manager->type == CHASSIS_CONF_MYSQL) {
    network_mysqld_con_send_ok_full(con->client, affected, 0,
                                    SERVER_STATUS_AUTOCOMMIT, 0);
...
void admin_select_connection_stat(network_mysqld_con* con, int backend_ndx, char *user)
{
  ...
  if (backend_ndx >= 0 && backend_ndx < network_backends_count(g->backends)) {
    ...
    GQueue *conns =
        network_connection_pool_get_conns(backend->pool, user_name, NULL);
    if (conns) {
...
void admin_select_user_password(network_mysqld_con* con, char* from_table, char *user)
{
  ...
  if (user) { /* one user */
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *)&username,
                                  (gpointer *)&hack)) {
...
static void admin_update_or_delete_remote_user_password_callback(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      network_mysqld_con_send_error(con->client,
                                    C("update remote user password timeout"));
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message("%s call send_admin_resp over, wait:%d", G_STRLOC,
                conf->ms_timeout);
      return;
    }
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_update_or_delete_remote_user_password_callback, "
              "timeout:%d",
              G_STRLOC, conf->ms_timeout);
    return;
  }

  if (!conf->options_success_flag) {
    network_mysqld_con_send_error(con->client,
                                  C("update remote user password failed"));
    con->is_admin_waiting_resp = 0;
    send_admin_resp(con->srv, con);
  ...
  network_mysqld_con_send_ok_full(con->client, 1, 0, SERVER_STATUS_AUTOCOMMIT,
                                  0);
...
static void admin_update_or_delete_remote_user_password(network_mysqld_con* con, chassis_config_t *conf)
{
  ...
  evtimer_set(&chas->remote_config_event,
              admin_update_or_delete_remote_user_password_callback, con);
...
void admin_update_user_password(network_mysqld_con *con, char *from_table,
  ...
  enum cetus_pwd_type pwd_type = password_type(from_table);
  gboolean affected =
      cetus_users_update_record(g->users, user, password, pwd_type);
...
void admin_insert_backend(network_mysqld_con* con, char *addr, char *type, char *state)
{
  ...
  int ret = network_backends_add(g->backends, addr, backend_type(type),
    ...
    if (con->srv->config_manager->type == CHASSIS_CONF_MYSQL) {
      ...
      if (backend_type(type) == BACKEND_TYPE_RO) {
        ...
        network_mysqld_con_send_error(
            con->client,
            C("insert backend succeed, but save to remote failed."));
      }
    } else {
      gint effected_rows = 0;
      ret = save_setting(con->srv, &effected_rows);
  ...
  case BACKEND_OPERATE_2MASTER: {
    network_mysqld_con_send_error(
        con->client,
        C("rw node is already exists，only one rw node is allowed"));
    break;
  }
  }
}
...
void admin_update_backend(network_mysqld_con *con, GList *equations,
  ...
  if (type_str && backend_type(type_str) == BACKEND_TYPE_RW &&
      network_backend_check_available_rw(g->backends)) {
  ...
  int affected = (network_backends_modify(g->backends, backend_ndx, type, state,
                                          NO_PREVIOUS_STATE) == 0)
                     ? 1
                     : 0;
  if (con->srv->config_manager->type == CHASSIS_CONF_MYSQL) {
...
void admin_delete_backend(network_mysqld_con* con, char *key, char *val)
{
  ...
  if (backend_ndx >= 0 && backend_ndx < network_backends_count(g->backends)) {
    network_backends_remove(g->backends,
                            backend_ndx); /*TODO: just change state? */
    if (con->srv->config_manager->type == CHASSIS_CONF_MYSQL) {
      chassis_config_t *conf = con->srv->config_manager;
      conf->key = g_strdup("proxy-read-only-backend-addresses");
      conf->value = admin_get_value_by_key(con, conf->key);
    ...
    network_mysqld_con_send_ok_full(con->client, 0, 0, SERVER_STATUS_AUTOCOMMIT,
                                    0);
  }
}
...
void admin_get_stats(network_mysqld_con* con, char* p)
{
  ...
  if (strcasecmp(p, "client_query") == 0) {
    ...
    for (i; i < backends_num && i < MAX_SERVER_NUM; ++i) {
      ...
      g_ptr_array_add(row, g_strdup(buffer));
      g_ptr_array_add(row,
                      g_strdup_printf("server_query_details.%d.ro", i + 1));
      g_ptr_array_add(
          row, g_strdup_printf("%lu", stats->server_query_details[i].ro));
      g_ptr_array_add(rows, row);
      row = g_ptr_array_new_with_free_func(g_free);
      g_ptr_array_add(row, g_strdup(buffer));
      g_ptr_array_add(row,
                      g_strdup_printf("server_query_details.%d.rw", i + 1));
      g_ptr_array_add(
          row, g_strdup_printf("%lu", stats->server_query_details[i].rw));
      g_ptr_array_add(rows, row);
    }
...
void admin_get_config(network_mysqld_con* con, char* p)
{
  ...
  if (strcasecmp(p, "common") == 0) {
    snprintf(buf1, 32, "%d", chas->long_query_time);
    APPEND_ROW_3_COL(rows, buffer, "common.long_query_time", buf1);
...
static void admin_set_remote_config_callback(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      network_mysqld_con_send_error(con->client,
                                    C("update remote config timeout"));
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message(
          "%s call admin_set_remote_config_callback timeout, last timeout:%d",
          G_STRLOC, conf->ms_timeout);
      return;
    }
    conf->ms_timeout = conf->ms_timeout << 1;
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_set_remote_config_callback, timeout:%d", G_STRLOC,
              conf->ms_timeout);
    return;
  }
  ...
  if (!conf->options_success_flag) {
    network_mysqld_con_send_error(
        con->client,
        C("Variable is set locally but cannot replace remote settings"));
    con->is_admin_waiting_resp = 0;
    send_admin_resp(con->srv, con);
    g_message("%s call send_admin_resp over", G_STRLOC);
    return;
...
static void admin_set_remote_config(network_mysqld_con* con, chassis_config_t *conf)
{
  ...
  evtimer_set(&chas->remote_config_event, admin_set_remote_config_callback,
              con);
  struct timeval check_interval = {0, 2000};
  conf->ms_timeout = 2;
  conf->retries = 1;
...
void admin_update_remote_backend_callback(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      network_mysqld_con_send_error(con->client,
                                    C("update remote backend timeout"));
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message("%s call admin_update_remote_backend_callback timeout, last "
                "timeout:%d",
                G_STRLOC, conf->ms_timeout);
      goto fun_end;
    }
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_update_remote_backend_callback, timeout:%d",
              G_STRLOC, conf->ms_timeout);
    return;
  }
  ...
  if (!conf->options_success_flag) {
    network_mysqld_con_send_error(
        con->client, C("backend is set locally but cannot replace remote"));
    con->is_admin_waiting_resp = 0;
    send_admin_resp(con->srv, con);
    g_message("%s call send_admin_resp over", G_STRLOC);
    goto fun_end;
...
static void admin_update_remote_backend(network_mysqld_con* con, chassis_config_t *conf)
{
  ...
  evtimer_set(&chas->remote_config_event, admin_update_remote_backend_callback,
              con);
  struct timeval check_interval = {0, 2000};
  conf->ms_timeout = 2;
  conf->retries = 1;
...
void admin_set_config(network_mysqld_con* con, char* key, char* value)
{
  ...
  param.chas = con->srv;
  param.opt_type = opt->opt_property;
  int ret = opt->assign_hook != NULL ? opt->assign_hook(value, &param)
                                     : ASSIGN_NOT_SUPPORT;
  ...
  if (0 == ret) {
    ...
    network_mysqld_con_send_error_full(
        con->client, C("Variable cannot be set dynamically"), 1065, "28000");
  } else if (ASSIGN_VALUE_INVALID == ret) {
    network_mysqld_con_send_error_full(con->client, C("Value is illegal"), 1065,
                                       "28000");
  } else {
    network_mysqld_con_send_error_full(
        con->client, C("You have an error in your SQL syntax"), 1065, "28000");
  }
}
...
static void admin_update_settings(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      network_mysqld_con_send_error(con->client,
                                    C("update remote settings timeout"));
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message("%s call admin_update_settings timeout, last timeout:%d",
                G_STRLOC, conf->ms_timeout);
      return;
    }
    conf->ms_timeout = conf->ms_timeout << 1;
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_update_settings, timeout:%d", G_STRLOC,
              conf->ms_timeout);
    return;
  }
...
static void admin_update_user(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      ...
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message("%s call send_admin_resp timeout, last timeout:%d", G_STRLOC,
                conf->ms_timeout);
      return;
    }
    conf->ms_timeout = conf->ms_timeout << 1;
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_update_user, timeout:%d", G_STRLOC,
              conf->ms_timeout);
    return;
  }
  ...
  char *buffer = NULL;
  if (chassis_config_query_object(conf, "users", &buffer, 0) == FALSE ||
      buffer == NULL) {
    if (buffer) {
      g_free(buffer);
    }
...
static void admin_update_variables(int fd, short what, void *arg)
{
  ...
  if (conf->options_update_flag) {
    ...
    if (conf->retries == 0) {
      ...
      con->is_admin_waiting_resp = 0;
      send_admin_resp(con->srv, con);
      g_message("%s call admin_update_variables timeout, last timeout:%d",
                G_STRLOC, conf->ms_timeout);
      return;
    }
    conf->ms_timeout = conf->ms_timeout << 1;
    ...
    chassis_event_add_with_timeout(chas, &chas->remote_config_event,
                                   &check_interval);
    g_message("%s call admin_update_variables, timeout:%d", G_STRLOC,
              conf->ms_timeout);
    return;
  }
  ...
  char *buffer = NULL;
  if (chassis_config_query_object(conf, "variables", &buffer, 0) == FALSE ||
      buffer == NULL) {
    if (buffer) {
      g_free(buffer);
    }
...
static struct sql_help_entry_t {
  ...
  int type;
    ...
    {"add allow_ip|deny_ip 'user@address'",
     "add address to white or black list", ALL_HELP},
    ...
    {"delete allow_ip|deny_ip 'user@address'",
     "delete address from white or black list", ALL_HELP},
    {"delete from user_pwd where user='name'",
     "delete from user_pwd where user='test'; ", ALL_HELP},
    {"delete from app_user_pwd where user='name'",
     "delete from user_pwd where user='test'; ", ALL_HELP},
    {"delete from backends where [backend_ndx=index|address='ip:port']",
     "e.g. delete from backends where backend_ndx = 1; ", ALL_HELP},
    {"insert into backends values ('ip:port', '[ro|rw]', 'state')",
     "add mysql instance to backends list", RW_HELP},
    {"refresh_conns", "refresh all server connections. e.g. refresh_conns; ",
     ALL_HELP},
    {"remove backend where [backend_ndx=index|address='ip:port']",
     "e.g. remove backend where address='3.1.2.1:6666'; ", ALL_HELP},
    {"remove backend backend_ndx", "e.g. remove backend 1; ", ALL_HELP},
    {"save settings",
     "save the current running configuration to files. e.g. save settings; ",
     ALL_HELP},
    ...
    {"select * from user_pwd [where user='name']",
     "e.g. select * from user_pwd; ", ALL_HELP},
    {"select * from app_user_pwd [where user='name']",
     "e.g. select * from app_user_pwd where user='test'; ", ALL_HELP},
    {"set reduce_conns [true|false]",
     "reduce idle connections if set to true. e.g. set reduce_conns true; ",
     ALL_HELP},
    {"set maintain [true|false]",
     "close all client connections if set to true. e.g. set maintain false; ",
     ALL_HELP},
    {"set charset_check [true|false]",
     "check the client charset is equal to the default charset", ALL_HELP},
    {"show allow_ip|deny_ip",
     "show allow_ip|deny_ip rules. e.g. show allow_ip; ", ALL_HELP},
    {"show connectionlist [num]",
     "show num connections. e.g. show connectionlist; ", ALL_HELP},
    {"show maintain status", "e.g. show maintain status; ", ALL_HELP},
    {"show variables [like '%pattern%']",
     "e.g. show variables like '%proxy%'; ", ALL_HELP},
    ...
    {"update user_pwd set password='xx' where user='name'",
     "e.g. update user_pwd set password='123' where user='test'; ", ALL_HELP},
    {"update app_user_pwd set password='xx' where user='name'",
     "e.g. update app_user_pwd set password='123' where user='test'; ",
     ALL_HELP},
    {"update backends set (type|state)=x where "
     "[backend_ndx=index|address='ip:port']",
...
static gint save_setting(chassis *srv, gint *effected_rows)
{
  ...
  if (ret == ASSIGN_OK) {
    ...
    *effected_rows = chassis_options_save(keyfile, srv->options, srv);
    ...
    GError *gerr = NULL;
    if (FALSE ==
        g_file_set_contents(srv->default_file, file_buf, file_size, &gerr)) {
      ret = SAVE_ERROR;
      g_clear_error(&gerr);
    } else {
      if ((ret = chmod(srv->default_file,
...
{
  if (ret == ASSIGN_OK) {
    network_mysqld_con_send_ok_full(client, affected, 0,
                                    SERVER_STATUS_AUTOCOMMIT, 0);
  } else {
    char *msg = NULL;
    switch (ret) {
    case SAVE_ERROR:
      ...
      break;
    case CHMOD_ERROR:
      msg = "chmod file failed";
      break;
    case CHANGE_SAVE_ERROR:
      msg = "change config and save file failed";
      break;
    default:
...
void admin_sql_log_start(network_mysqld_con* con) {
  ...
  if (con->srv->sql_mgr->sql_log_switch == OFF) {
    network_mysqld_con_send_error(
        con->client,
        C("can not start sql log thread, because sql-log-switch = OFF"));
    return;
  }
...
void admin_sql_log_stop(network_mysqld_con* con) {
  ...
  if (con->srv->sql_mgr->sql_log_action == SQL_LOG_START) {
    ...
    network_mysqld_con_send_error(con->client,
                                  C("sql log thread has been stopped"));
  }
}
...
void admin_sql_log_status(network_mysqld_con* con) {
  ...
  for (l = options; l; l = l->next) {
    ...
    if (sql_pattern_like(pattern, opt->long_name)) {
      ...
      freelist = g_list_append(freelist, value);
      APPEND_ROW_4_COL(
          rows, buffer, (char *)opt->long_name, value,
          (CAN_ASSIGN_OPTS_PROPERTY(opt->opt_property) ? "Dynamic" : "Static"));
    }
  }

  APPEND_ROW_4_COL(rows, buffer, "sql-log-state",
                   con->srv->sql_mgr->sql_log_action == SQL_LOG_START
                       ? "running"
                       : "stopped",
                   "Internal");
  gchar *cached = NULL;
  if (con->srv->sql_mgr->fifo &&
      (con->srv->sql_mgr->sql_log_action == SQL_LOG_START)) {
    cached = g_strdup_printf("%u", con->srv->sql_mgr->fifo->in -
                                       con->srv->sql_mgr->fifo->in);
  } else {
  ...
  APPEND_ROW_4_COL(rows, buffer, "sql-log-fullname",
                   con->srv->sql_mgr->sql_log_fullname == NULL
                       ? "NULL"
                       : con->srv->sql_mgr->sql_log_fullname,
                   "Internal");

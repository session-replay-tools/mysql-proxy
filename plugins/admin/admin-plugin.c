...
NETWORK_MYSQLD_PLUGIN_PROTO(server_read_auth) {
  ...
  if (con->client->response == NULL) {
    auth =
        network_mysqld_auth_response_new(con->client->challenge->capabilities);
...
#ifdef HAVE_OPENSSL
    if (auth->ssl_request) {
      if (network_ssl_create_connection(con->client, NETWORK_SSL_SERVER) ==
          FALSE) {
  ...
  if (!strleq(S(con->client->response->username), con->config->admin_username,
              strlen(con->config->admin_username))) {
    network_mysqld_con_send_error_full(send_sock, C("unknown user"), 1045,
                                       "28000");

    /* close the connection after we have sent this packet */
    con->state = ST_SEND_ERROR;
  } else if (network_mysqld_proto_password_hash(
                 hashed_pwd, con->config->admin_password,
                 strlen(con->config->admin_password))) {
  } else if (network_mysqld_proto_password_scramble(
                 excepted_response, S(recv_sock->challenge->auth_plugin_data),
                 S(hashed_pwd))) {
    network_mysqld_con_send_error_full(send_sock, C("scrambling failed"), 1045,
                                       "28000");

    /* close the connection after we have sent this packet */
    con->state = ST_SEND_ERROR;
  } else if (!g_string_equal(excepted_response, auth->auth_plugin_data)) {
    network_mysqld_con_send_error_full(send_sock, C("password doesn't match"),
                                       1045, "28000");
...
static gboolean cetus_result_retrieve_field_count(GQueue *input,
                                                  guint64 *p_field_count) {
...
static int check_field_count_consistant(GPtrArray *recv_queues,
  ...
  for (i = 0; i < recv_queues->len; i++) {
    ...
    if (cetus_result_retrieve_field_count(queue->chunks, field_count) ==
        FALSE) {
...
static int prepare_for_row_process(GList **candidates, GPtrArray *recv_queues,
                                   network_queue *send_queue, guint pkt_count,
                                   result_merge_t *merged_result) {
  ...
  for (i = 0; i < recv_queues->len; i++) {
    ...
    for (j = pkt_count; j > 0; --j) {
      ...
      if (packet == NULL) {
        g_warning("%s:packet null, pkt cnt:%d", G_STRLOC, pkt_count);
        merged_result->status = RM_FAIL;
        return 0;
      }
    ...
    if (rows_start) {
      ...
      g_warning("%s:rows start null, pkt cnt:%d", G_STRLOC, pkt_count);
      merged_result->status = RM_FAIL;
      return 0;
    }
  }
...
static int do_simple_merge(network_mysqld_con *con, GPtrArray *recv_queues,
                           network_queue *send_queue, GList **candidates,
                           guint *pkt_count) {
...
static int merge_for_admin(network_mysqld_con *con, GPtrArray *recv_queues,
                           network_queue *send_queue, cetus_result_t *res_merge,
                           result_merge_t *merged_result) {
  int p;
  ...
  if (!prepare_for_row_process(candidates, recv_queues, send_queue, pkt_count,
                               merged_result)) {
  ...
  if (!do_simple_merge(con, recv_queues, send_queue, candidates, &pkt_count)) {
  ...
  GString *eof_pkt =
      g_string_new_len("\x05\x00\x00\x07\xfe\x00\x00\x02\x00", 9);
...
static int merge_for_modify(network_mysqld_con *con, GPtrArray *recv_queues,
                            network_queue *send_queue,
                            cetus_result_t *res_merge,
                            result_merge_t *merged_result) {
  ...
  network_mysqld_con_send_ok_full(con->client, total_affected_rows, 0, 0x02,
                                  total_warnings);

  return 1;
}

void admin_resultset_merge(network_mysqld_con *con, GPtrArray *recv_queues,
                           network_queue *send_queue,
                           result_merge_t *merged_result) {
  ...
  if (con->admin_read_merge) {
    ...
    if (!merge_for_admin(con, recv_queues, send_queue, &res_merge,
                         merged_result)) {
      cetus_result_destroy(&res_merge);
      return;
    }
    ...
    if (!merge_for_modify(con, recv_queues, send_queue, &res_merge,
                          merged_result)) {
      cetus_result_destroy(&res_merge);
      return;
    }
...
static void network_read_sql_resp(int G_GNUC_UNUSED fd, short events,
                                  void *user_data) {
  ...
  g_debug("%s: channel command: %u, need to read servers:%d", G_STRLOC,
          ch.basics.command, con->num_read_pending);
  ...
  if (ch.basics.command == CETUS_CMD_ADMIN_RESP) {
    ...
    do {
      ...
      if (len > 0) {
        g_debug("%s: resp_len:%d, len:%d, fd:%d", G_STRLOC, unread_len, len,
                fd);
        ...
        g_message("%s: resp_len:%d, len:%d, fd:%d", G_STRLOC, unread_len, len,
                  fd);
      }
    } while (unread_len > 0);
  ...
  if (con->num_read_pending == 0) {
    ...
    admin_resultset_merge(con, recv_queues, con->client->send_queue, &result);
...
static int construct_channel_info(network_mysqld_con *con, char *sql) {
  chassis *cycle = con->srv;
  g_debug("%s:call construct_channel_info, cetus_process_slot:%d", G_STRLOC,
          cetus_process_slot);
  cetus_channel_t ch;
  ...
  if (len >= MAX_ADMIN_SQL_LEN) {
    ...
    g_debug("%s:cetus_last_process:%d, ch admin sql:%s", G_STRLOC,
            cetus_last_process, ch.admin_sql);
    if (con->ask_the_given_worker) {
      ...
      /* TODO: AGAIN */
      cetus_write_channel(cetus_processes[index].parent_child_channel[0], &ch,
                          sizeof(cetus_channel_t));
      int fd = cetus_processes[index].parent_child_channel[0];
      g_debug("%s:fd:%d for network_read_sql_resp", G_STRLOC, fd);
      event_set(&(cetus_processes[index].event), fd, EV_READ,
                network_read_sql_resp, con);
      chassis_event_add_with_timeout(cycle, &(cetus_processes[index].event),
                                     NULL);
      con->num_read_pending++;
    ...
    for (i = 0; i < num; i++) {
      ...
      if (fd > 0) {
        ...
        /* TODO: AGAIN */
        cetus_write_channel(cetus_processes[i].parent_child_channel[0], &ch,
                            sizeof(cetus_channel_t));
        g_debug("%s:fd:%d for network_read_sql_resp", G_STRLOC, fd);
        event_set(&(cetus_processes[i].event), fd, EV_READ,
                  network_read_sql_resp, con);
        chassis_event_add_with_timeout(cycle, &(cetus_processes[i].event),
                                       NULL);
        con->num_read_pending++;
        ...
        g_message("%s:fd is not valid:%d, num:%d, pending:%d", G_STRLOC, fd,
                  num, con->num_read_pending);
      }
    }
    g_debug("%s:con num_read_pending:%d", G_STRLOC, con->num_read_pending);
...
static void network_send_admin_sql_to_workers(int G_GNUC_UNUSED fd, short what,
                                              void *user_data) {
  network_mysqld_con *con = user_data;

  if (con->srv->socketpair_mutex) {
    event_set(&con->client->event, con->client->fd, EV_TIMEOUT,
              network_send_admin_sql_to_workers, con);
...
NETWORK_MYSQLD_PLUGIN_PROTO(server_read_query) {
  ...
  case PROXY_NO_DECISION:
    network_mysqld_con_send_error(
        con->client, C("request error, \"select * from help\" for usage"));
    con->state = ST_SEND_QUERY_RESULT;
    break;
  ...
  default:
    network_mysqld_con_send_error(
        con->client, C("network packet error, closing connection"));
    con->state = ST_SEND_ERROR;
    break;
  }
...
show_admin_username(gpointer param) {
  ...
  if (CAN_SHOW_OPTS_PROPERTY(opt_type)) {
    return g_strdup_printf(
        "%s", config->admin_username != NULL ? config->admin_username : "NULL");
  }
  if (CAN_SAVE_OPTS_PROPERTY(opt_type)) {
...
network_mysqld_admin_plugin_get_options(chassis_plugin_config *config) {
  ...
  chassis_options_add(
      &opts, "admin-address", 0, 0, OPTION_ARG_STRING, &(config->address),
      "listening address:port of the admin-server (default: :4041)",
      "<host:port>", NULL, show_admin_address,
      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "admin-username", 0, 0, OPTION_ARG_STRING,
                      &(config->admin_username), "username to allow to log in",
                      "<string>", NULL, show_admin_username,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "admin-password", 0, 0, OPTION_ARG_STRING,
                      &(config->admin_password), "password to allow to log in",
                      "<string>", NULL, show_admin_password,
                      SHOW_OPTS_PROPERTY | SAVE_OPTS_PROPERTY);

  chassis_options_add(&opts, "admin-allow-ip", 0, 0, OPTION_ARG_STRING,
                      &(config->allow_ip),
                      "ip address allowed to connect to admin", "<string>",
                      NULL, NULL, SAVE_OPTS_PROPERTY);
  chassis_options_add(&opts, "admin-deny-ip", 0, 0, OPTION_ARG_STRING,
                      &(config->deny_ip),
                      "ip address denyed to connect to admin", "<string>", NULL,
                      NULL, SAVE_OPTS_PROPERTY);
...
static int check_allowed_running(chassis *chas) {
...
network_mysqld_admin_plugin_stop_listening(chassis *chas,
  ...
  if (config->listen_con) {
    g_message("%s:close listen socket:%d", G_STRLOC,
              config->listen_con->server->fd);
    network_socket_free(config->listen_con->server);
    config->listen_con = NULL;

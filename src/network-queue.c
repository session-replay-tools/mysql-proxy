 ...
 */
GString *network_queue_peek_str(network_queue *queue, gsize peek_len,
                                GString *dest) {
   ...
   * g_debug("[%s] looking for %d bytes, queue has %d", G_STRLOC, peek_len,
   * queue->len);
   */
 ...
 */
GString *network_queue_pop_str(network_queue *queue, gsize steal_len,
                               GString *dest) {

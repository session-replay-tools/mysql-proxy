...
int network_pool_add_idle_conn(network_connection_pool *pool, chassis *srv,
  ...
  surplus_time =
      srv->max_alive_time - surplus_time + g_random_int_range(0, 240);
...
int network_pool_add_conn(network_mysqld_con *con, int is_swap) {
  ...
  if (con->prepare_stmt_count > 0) {
    g_debug("%s: con valid_prepare_stmt_cnt:%d for con:%p", G_STRLOC,
            con->prepare_stmt_count, con);
    return -1;
  }
  ...
  if (con->server_in_tran_and_auto_commit_received) {
    g_critical("%s:server in tran and auto commit and put to pool:%p", G_STRLOC,
               con);
  ...
  if (to_be_put_to_pool == TRUE && !is_swap && con->servers == NULL) {
    if (con->srv->is_reduce_conns) {
      if (network_conn_pool_do_reduce_conns_verdict(
              st->backend->pool, st->backend->connected_clients)) {
        to_be_put_to_pool = FALSE;
      }
    }
  }
  ...
  if (to_be_put_to_pool == FALSE) {
    if (con->server->recv_queue->chunks->length > 0) {
      g_critical("%s: recv queue length :%d, state:%s", G_STRLOC,
                 con->server->recv_queue->chunks->length,
    ...
    for (i = 0; i < MAX_SERVER_NUM_FOR_PREPARE; i++) {
      ...
      if (st->backend_ndx_array == NULL) {
        ...
        if (st->backend_ndx_array[i] <= 0) {
          g_message("%s: i:%d backend_ndx_array value:%d for con:%p", G_STRLOC,
                    i, st->backend_ndx_array[i], con);
          continue;
        }
        ...
        if (con->srv->server_conn_refresh_time <= server->create_time) {
          g_debug("%s: add conn fd:%d to pool:%p ", G_STRLOC, server->fd,
                  backend->pool);
        ...
        g_debug("%s, con:%p, backend ndx:%d:connected_clients sub, clients:%d",
                G_STRLOC, con, st->backend_ndx_array[i],
                backend->connected_clients);
        checked++;
...
void mysqld_con_reserved_connections_add(network_mysqld_con *con,
  ...
  if (st->backend_ndx_array == NULL) {
    ...
    g_debug("%s: set st->backend_ndx:%d ndx array 1", G_STRLOC,
            st->backend_ndx);
  }
  ...
  g_debug("%s: set backend_ndx:%d ndx array:%d", G_STRLOC, backend_idx,
          con->servers->len);
}

network_socket *mysqld_con_reserved_connections_get(network_mysqld_con *con,
                                                    int backend_idx) {
  proxy_plugin_con_t *st = con->plugin_con_state;
  if (con->servers) {
 ...
 */
network_socket *network_connection_pool_swap(network_mysqld_con *con,
                                             int backend_ndx) {
  ...
  g_debug("%s: check server switch for conn:%p, prep_stmt_cnt:%d, orig ndx:%d, "
          "now:%d",
  ...
  if (st->backend_ndx != -1 && con->prepare_stmt_count > 0 &&
    ...
    network_socket *sock =
        mysqld_con_reserved_connections_get(con, backend_ndx);
    if (sock) {
      return sock;
    }
  }
  ...
  GString *name =
      con->client->response ? con->client->response->username : &empty_name;
  network_socket *sock =
      network_connection_pool_get(backend->pool, name, &is_robbed);
  if (sock == NULL) {
    if (server_switch_need_add) {
      g_message("%s:retrieve master conn failed, but still hold read server",
                G_STRLOC);
      return NULL;
    }
  ...
  if (server_switch_need_add) {
    ...
    if (con->server) {
      if (network_pool_add_conn(con, 1) != 0) {
        g_debug("%s: take and move the current backend into the pool failed",
                G_STRLOC);
        return NULL;
      } else {
        g_debug("%s: take and move the current backend into the pool",
                G_STRLOC);
      }
    }
  }
  ...
  g_debug(
      "%s, con:%p, backend ndx:%d:connected_clients add, clients:%d, sock:%p",

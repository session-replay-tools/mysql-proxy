...
static gboolean url_parse_user_pass(chassis_config_t *rconf,
                                    const char *userpass, int len) {
...
static gboolean url_parse_host_port(chassis_config_t *rconf,
                                    const char *hostport, int len) {
...
static gboolean url_parse_parameter(chassis_config_t *rconf, const char *param,
                                    int len) {
...
static gboolean chassis_config_parse_mysql_url(chassis_config_t *rconf,
  ...
  if (schema) {
    if (param && param < schema)
      return FALSE;
    rconf->schema =
        param ? strndup(schema + 1, param - schema - 1) : strdup(schema + 1);
  }
  if (param)
...
static MYSQL *chassis_config_get_mysql_connection(chassis_config_t *conf) {
  ...
  if (mysql_real_connect(conn, conf->host, conf->user, conf->password, NULL,
                         conf->port, NULL, 0) == NULL) {
...
static gboolean chassis_config_mysql_init_tables(chassis_config_t *conf) {
  ...
  snprintf(sql, sizeof(sql),
           "CREATE TABLE IF NOT EXISTS %s.objects("
...
/* TODO: */
chassis_config_t *chassis_config_from_local_dir(char *conf_dir,
                                                char *conf_file) {
...
gboolean chassis_config_set_remote_options(chassis_config_t *conf, gchar *key,
                                           gchar *value) {
  ...
  snprintf(sql, sizeof(sql),
           "INSERT INTO %s.`settings`(option_key,option_value) VALUES ('%s', "
           "'%s') ON DUPLICATE KEY UPDATE option_value = '%s'",
           conf->schema, key, real_value, real_value);
  if (mysql_query(conn, sql)) {
...
gboolean chassis_config_set_remote_backends(chassis_config_t *conf, gchar *key1,
                                            gchar *value1, gchar *key2,
                                            gchar *value2) {
  if (conf->type != CHASSIS_CONF_MYSQL) {
    return TRUE;
  }
  ...
  if (key1) {
    if (value1) {
      ...
      snprintf(sql1, sizeof(sql1),
               "INSERT INTO %s.`settings`(option_key,option_value) VALUES "
               "('%s', '%s') ON DUPLICATE KEY UPDATE option_value = '%s'",
               conf->schema, key1, real_value1, real_value1);
    } else {
      snprintf(sql1, sizeof(sql1),
               "DELETE FROM %s.`settings` where option_key = '%s'",
               conf->schema, key1);
    }
  ...
  if (status1 && key2) {
    if (value2) {
      ...
      snprintf(sql2, sizeof(sql2),
               "INSERT INTO %s.`settings`(option_key,option_value) VALUES "
               "('%s', '%s') ON DUPLICATE KEY UPDATE option_value = '%s'",
               conf->schema, key2, real_value2, real_value2);
    } else {
      snprintf(sql2, sizeof(sql2),
               "DELETE FROM %s.`settings` where option_key = '%s'",
               conf->schema, key2);
...
struct config_object_t *chassis_config_get_object(chassis_config_t *conf,
                                                  const char *name) {
  GList *l = NULL;
  GList *objects;
...
gboolean chassis_config_mysql_query_object(chassis_config_t *conf,
...
gboolean chassis_config_query_object(chassis_config_t *conf, const char *name,
                                     char **json_res, int refresh) {
...
gboolean chassis_config_mysql_write_object(chassis_config_t *conf,
                                           struct config_object_t *object,
  ...
  g_string_printf(sql,
                  "INSERT INTO %s.objects(object_name,object_value,mtime)"
                  " VALUES('%s','%s', FROM_UNIXTIME(%ld)) ON DUPLICATE KEY "
                  "UPDATE object_value = '%s', mtime = FROM_UNIXTIME(%ld)",
                  conf->schema, name, json, now, json, now);
...
gboolean chassis_config_write_object(chassis_config_t *conf, const char *name,
                                     const char *json) {
  struct config_object_t *object = chassis_config_get_object(conf, name);
  if (!object) {
    object = g_new0(struct config_object_t, 1);
    ...
    if (entry_value) {
      ...
      case OPTION_ARG_NONE:
        if (entry->arg_data == NULL)
          break;
        if (strcasecmp(entry_value, "false") == 0 ||
            strncmp(entry_value, "0", 1) == 0) {
          *(int *)(entry->arg_data) = 0;
        } else if (strcasecmp(entry_value, "true") == 0 ||
                   isdigit(entry_value[0])) {
...
gboolean chassis_config_mysql_is_object_outdated(chassis_config_t *conf,
  ...
  snprintf(sql, sizeof(sql),
           "SELECT mtime FROM %s.objects where object_name='%s'", conf->schema,
           name);
  gboolean status = FALSE;
  if (mysql_query(conn, sql)) {
    g_warning("sql failed: %s", sql);
...
gboolean chassis_config_is_object_outdated(chassis_config_t *conf,
                                           const char *name) {
  struct config_object_t *object = chassis_config_get_object(conf, name);
  if (!object) {
    return FALSE;
...
gboolean chassis_config_register_service(chassis_config_t *conf, char *id,
                                         char *data) {
  if (conf->type != CHASSIS_CONF_MYSQL)
    return FALSE;
  ...
  time_t now = time(0);
  snprintf(sql, sizeof(sql),
           "INSERT INTO %s.services(id, data, start_time)"
...
gboolean chassis_config_reload_variables(chassis_config_t *conf,
                                         const char *name) {
  ...
  if (!conn) {
    g_warning("%s:Cannot connect to mysql server when reload variables.",
              G_STRLOC);
    return FALSE;
  }
  char sql[256] = {0};
  snprintf(sql, sizeof(sql),
           ...
           conf->schema, name);
  if (mysql_query(conn, sql)) {
    g_warning("sql failed: %s, when reload variables, mysql_errno: %d", sql,
              mysql_errno(conn));
    goto recycle_mysql_resources;
  }
  MYSQL_RES *result = mysql_store_result(conn);

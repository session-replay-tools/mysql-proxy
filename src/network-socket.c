/* $%BEGINLICENSE%$
   Copyright (c) 2007, 2012, 2021 Oracle and/or its affiliates. All rights
   reserved.
...
void network_socket_send_quit_and_free(network_socket *s) {
...
network_socket_retval_t network_socket_set_non_blocking(network_socket *sock) {
  ...
  if (ret != 0) {
    g_critical("%s: set_non_blocking() failed: %s (%d)", G_STRLOC,
...
network_socket_set_send_buffer_size(network_socket *sock, int size) {
  ...
  if (setsockopt(sock->fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof(size)) != 0) {
    g_critical("%s: setsockopt SO_SNDBUF failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
    return NETWORK_SOCKET_ERROR;
  }
...
network_socket *network_socket_accept(network_socket *srv, int *reason) {
...
#else
  if (-1 == (client->fd = accept(srv->fd, &client->src->addr.common,
                                 &(client->src->len)))) {
    *reason = errno;
    network_socket_free(client);
    return NULL;
   ...
   */
  if (-1 ==
      getsockname(client->fd, &client->dst->addr.common, &(client->dst->len))) {
...
network_socket_connect_setopts(network_socket *sock) {
  ...
  if (setsockopt(sock->fd, IPPROTO_IP, IP_TOS, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt IP_TOS failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
#endif
  val = 1;
  if (setsockopt(sock->fd, IPPROTO_TCP, TCP_NODELAY, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt TCP_NODELAY failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
  val = 1;
  if (setsockopt(sock->fd, SOL_SOCKET, SO_KEEPALIVE, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt SO_KEEPALIVE failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
  val = 30;
  if (setsockopt(sock->fd, SOL_TCP, TCP_KEEPIDLE, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt TCP_KEEPIDLE failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
  val = 5;
  if (setsockopt(sock->fd, SOL_TCP, TCP_KEEPINTVL, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt TCP_KEEPINTVL failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
  val = 3;
  if (setsockopt(sock->fd, SOL_TCP, TCP_KEEPCNT, &val, sizeof(val)) != 0) {
    g_critical("%s: setsockopt TCP_KEEPCNT failed: %s (%d)", G_STRLOC,
               g_strerror(errno), errno);
  }
  ...
  if (-1 == getsockname(sock->fd, &sock->src->addr.common, &(sock->src->len))) {
    g_debug("%s: getsockname() failed: %s (%d)", G_STRLOC, g_strerror(errno),
            errno);
...
network_socket_retval_t network_socket_connect_finish(network_socket *sock) {
  ...
  if (getsockopt(sock->fd, SOL_SOCKET, SO_ERROR, &so_error, &so_error_len)) {
    /* getsockopt failed */
    g_critical("%s: getsockopt(%s) failed: %s (%d)", G_STRLOC,
...
network_socket_retval_t network_socket_connect(network_socket *sock) {
  ...
  if (-1 == (sock->fd = socket(sock->dst->addr.common.sa_family,
                               sock->socket_type, 0))) {
    g_critical("%s: socket(%s) failed: %s (%d)", G_STRLOC, sock->dst->name->str,
               g_strerror(errno), errno);
    return NETWORK_SOCKET_ERROR;
  }
  ...
  if (-1 == connect(sock->fd, &sock->dst->addr.common, sock->dst->len)) {
    ...
    case E_NET_WOULDBLOCK:
      return NETWORK_SOCKET_ERROR_RETRY;
    default:
      g_critical("%s: connect(%s) failed: %s (%d)", G_STRLOC,
                 sock->dst->name->str, g_strerror(errno), errno);
      return NETWORK_SOCKET_ERROR;
    }
  }
...
static void attach_bpf(int fd) 
{
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_ATTACH_REUSEPORT_CBPF, &p, sizeof(p))) {
    g_critical("%s:failed to set SO_ATTACH_REUSEPORT_CBPF, err:%s", G_STRLOC,
               strerror(errno));
  }
}
#endif
...
#define SETSOCKOPT_OPTVAL_CAST (void *)
  ...
  if (con->socket_type == SOCK_STREAM) {
    ...
    if (-1 == (con->fd = socket(con->dst->addr.common.sa_family,
                                con->socket_type, 0))) {
      g_critical("%s: socket(%s) failed: %s (%d)", G_STRLOC,
                 con->dst->name->str, g_strerror(errno), errno);
      return NETWORK_SOCKET_ERROR;
    }
    ...
    if (con->dst->addr.common.sa_family == AF_INET ||
      ...
      if (0 != setsockopt(con->fd, IPPROTO_TCP, TCP_NODELAY,
        ...
        g_critical(
            "%s: setsockopt(%s, IPPROTO_TCP, TCP_NODELAY) failed: %s (%d)",
      ...
      if (0 != setsockopt(con->fd, SOL_SOCKET, SO_REUSEADDR,
                          SETSOCKOPT_OPTVAL_CAST & val, sizeof(val))) {
        g_critical(
            "%s: setsockopt(%s, SOL_SOCKET, SO_REUSEADDR) failed: %s (%d)",
            G_STRLOC, con->dst->name->str, g_strerror(errno), errno);
        return NETWORK_SOCKET_ERROR;
      }
      ...
      if (advanced_mode) {
        ...
        if (0 != setsockopt(con->fd, SOL_SOCKET, SO_REUSEPORT,
                            SETSOCKOPT_OPTVAL_CAST & val, sizeof(val))) {
          g_critical(
              "%s: setsockopt(%s, SOL_SOCKET, SO_REUSEPORT) failed: %s (%d)",
              G_STRLOC, con->dst->name->str, g_strerror(errno), errno);
          return NETWORK_SOCKET_ERROR;
        }
    ...
    if (con->dst->addr.common.sa_family == AF_INET6) {
      ...
      if (0 != setsockopt(con->fd, IPPROTO_IPV6, IPV6_V6ONLY,
                          SETSOCKOPT_OPTVAL_CAST & val, sizeof(val))) {
        g_critical(
            "%s: setsockopt(%s, IPPROTO_IPV6, IPV6_V6ONLY) failed: %s (%d)",
            G_STRLOC, con->dst->name->str, g_strerror(errno), errno);
        return NETWORK_SOCKET_ERROR;
      }
#endif
    ...
    if (-1 == bind(con->fd, &con->dst->addr.common, con->dst->len)) {
      ...
      if (-1 == connect(con->fd, &con->dst->addr.common, con->dst->len)) {
        g_debug("%s: connect(%s) failed: %s (%d)", G_STRLOC,
                con->dst->name->str, g_strerror(errno), errno);
        ...
        if (-1 == bind(con->fd, &con->dst->addr.common, con->dst->len)) {
          g_critical("%s: bind(%s) failed: %s (%d)", G_STRLOC,
                     con->dst->name->str, g_strerror(errno), errno);
    ...
    if (con->dst->addr.common.sa_family == AF_INET &&
      ...
      if (0 != getsockname(con->fd, (struct sockaddr *)&a, &a_len)) {
        g_critical("%s: getsockname(%s) failed: %s (%d)", G_STRLOC,
                   con->dst->name->str, g_strerror(errno), errno);
        return NETWORK_SOCKET_ERROR;
      }
    ...
    if (-1 == listen(con->fd, 128)) {
      g_critical("%s: listen(%s, 128) failed: %s (%d)", G_STRLOC,
                 con->dst->name->str, g_strerror(errno), errno);
      return NETWORK_SOCKET_ERROR;
    }
  } else {
    ...
    if (-1 == (con->fd = socket(con->src->addr.common.sa_family,
                                con->socket_type, 0))) {
      g_critical("%s: socket(%s) failed: %s (%d)", G_STRLOC,
                 con->src->name->str, g_strerror(errno), errno);
      return NETWORK_SOCKET_ERROR;
    }

    if (-1 == bind(con->fd, &con->src->addr.common, con->src->len)) {
      g_critical("%s: bind(%s) failed: %s (%d)", G_STRLOC, con->src->name->str,
                 g_strerror(errno), errno);
      return NETWORK_SOCKET_ERROR;
    }
  }
...
network_socket_retval_t network_socket_read(network_socket *sock) {
  ...
  if (sock->to_read > 0) {
    ...
    g_debug("%s: recv queue length:%d, sock:%p, client addr:%s, to read:%d",
            G_STRLOC, sock->recv_queue_raw->chunks->length, sock,
            sock->src->name->str, (int)sock->to_read);

    g_debug("%s: tcp read:%d for fd:%d", G_STRLOC, (int)sock->to_read,
            sock->fd);
    ...
    if (-1 == len) {
      ...
      case EAGAIN:
        g_message("%s:server to read:%d, but empty", G_STRLOC,
                  (int)sock->to_read);
 ...
 *
 */
static network_socket_retval_t network_socket_write_writev(network_socket *con,
                                                           int send_chunks) {
  ...
  network_queue *send_queue =
      con->do_compress ? con->send_queue_compressed : con->send_queue;

  chunk_count =
      send_chunks > 0 ? send_chunks : (gint)send_queue->chunks->length;
  ...
  g_debug("%s: network socket:%p, send (src:%s, dst:%s) fd:%d", G_STRLOC, con,
          con->src->name->str, con->dst->name->str, con->fd);

  len = writev(con->fd, iov, chunk_id);
  g_debug("%s: tcp write:%d, chunk count:%d", G_STRLOC, (int)len,
          (int)chunk_id);
  os_errno = errno;
  ...
  if (-1 == len) {
    ...
    case E_NET_CONNABORTED:
      ...
      return NETWORK_SOCKET_ERROR;
    default:
      g_message("%s: writev(%s, ...) failed: %s", G_STRLOC, con->dst->name->str,
                g_strerror(os_errno));
      return NETWORK_SOCKET_ERROR;
    }
  } else if (len == 0) {
 ...
 */
network_socket_retval_t network_socket_write(network_socket *sock,
                                             int send_chunks) {
...
network_socket_retval_t network_socket_to_read(network_socket *sock) {
  ...
  if (0 != ioctl(sock->fd, FIONREAD, &b)) {
    g_critical("%s: ioctl(%d, FIONREAD, ...) failed: %s (%d)", G_STRLOC,
               sock->fd, g_strerror(errno), errno);
    return NETWORK_SOCKET_ERROR;
  } else if (b < 0) {

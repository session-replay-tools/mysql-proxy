...
void chassis_event_add_with_timeout(chassis *chas, struct event *ev,
                                    struct timeval *tv) {
...
chassis_event_loop_t *chassis_event_loop_new() { return event_base_new(); }

void chassis_event_loop_free(chassis_event_loop_t *event) {
  event_base_free(event);
}

void *chassis_event_loop(chassis_event_loop_t *loop, int *mutex) {
  ...
  while ((mutex != NULL && (*mutex) != 0) || !chassis_is_shutdown()) {
    ...
    if (r == -1) {
      g_debug("%s: after event_base_dispatch:%d, errno:%d, str:%s", G_STRLOC, r,
      ...
      g_critical(
          "%s: leaving chassis_event_loop early, errno != EINTR was: %s (%d)",

...
int network_mysqld_proto_get_lenenc_int(network_packet *packet, guint64 *v) {
  ...
  if (off >= packet->data->len) {
    g_critical("%s: offset is too large:%d, packet len:%d", G_STRLOC, (int)off,
               (int)packet->data->len);
  ...
  if (bytestream[off] < 251) { /* */
    ...
    if (off + 8 >= packet->data->len)
      return -1;
    ret =
        (unsigned int)((bytestream[off + 5] << 0) | (bytestream[off + 6] << 8) |
                       (bytestream[off + 7] << 16) |
                       (bytestream[off + 8] << 24));
    ret <<= 32;

    ret +=
        (unsigned int)((bytestream[off + 1] << 0) | (bytestream[off + 2] << 8) |
                       (bytestream[off + 3] << 16) |
                       (bytestream[off + 4] << 24));
 ...
 */
int network_mysqld_proto_peek_int_len(network_packet *packet, guint64 *v,
                                      gsize size) {
  gsize i;
  int shift;
  guint32 r_l = 0, r_h = 0;
...
int network_mysqld_proto_get_int_len(network_packet *packet, guint64 *v,
                                     gsize size) {
  int err = 0;
...
int network_mysqld_proto_get_int8(network_packet *packet, guint8 *v) {
  ...
  g_assert_cmpint(v64 & 0xff, ==,
                  v64); /* check that we really only got one byte back */
 ...
 */
int network_mysqld_proto_get_str_len(network_packet *packet, gchar **s,
                                     gsize len) {
  gchar *str;
...
static int network_mysqld_proto_get_str_len2(network_packet *packet, gchar *s,
                                             gsize len) {
  if (len == 0) {
    return 0;
  }
 ...
 */
int network_mysqld_proto_get_lenenc_str(network_packet *packet, gchar **s,
                                        guint64 *_len) {
  guint64 len;
...
int network_mysqld_proto_get_column(network_packet *packet, gchar *s,
                                    gsize s_size) {
  guint64 len;
  ...
  if (s_size <= len) {
    g_critical("%s: column too long:%ld, buffer size:%ld", G_STRLOC, len,
               s_size);
    return -1;
  }
...
int network_mysqld_proto_get_string(network_packet *packet, gchar **s) {
  ...
  for (len = 0; packet->offset + len < packet->data->len &&
                *(packet->data->str + packet->offset + len);
       len++)
    ;
 ...
 */
int network_mysqld_proto_get_gstr_len(network_packet *packet, gsize len,
                                      GString *out) {
  int err = 0;
  ...
  err = err || (packet->offset >=
                packet->data->len); /* the offset is already too large */
  err = err || (packet->offset + len >
                packet->data->len); /* offset would get too large */
...
int network_mysqld_proto_get_gstr(network_packet *packet, GString *out) {
  ...
  for (len = 0; packet->offset + len < packet->data->len &&
                *(packet->data->str + packet->offset + len) != '\0';
       len++)
    ;

  if (packet->offset + len ==
      packet->data->len) { /* havn't found a trailing \0 */
    return -1;
  }
...
int network_mysqld_proto_set_compressed_packet_len(GString *_header,
                                                   guint32 length,
                                                   guint32 len_before) {
  unsigned char *header = (unsigned char *)_header->str;
 ...
 * @return 0
 */
int network_mysqld_proto_append_lenenc_str_len(GString *packet, const char *s,
                                               guint64 length) {
  if (!s) {
...
int network_mysqld_proto_append_lenenc_str(GString *packet, const char *s) {
  return network_mysqld_proto_append_lenenc_str_len(packet, s,
                                                    s ? strlen(s) : 0);
}
 ...
 * @return        0
 */
static int network_mysqld_proto_append_int_len(GString *packet, guint64 num,
                                               gsize size) {
  gsize i;
...
int network_mysqld_proto_password_scramble(GString *response,
  ...
  /* if the challenge is 21 bytes long it means we're behind a 5.5.7 or up
   * server that supports authentication plugins. After auth-plugin-data-2 the
   * protocol adds a spacing character to split it from the next part of the
   * packet: auth-plugin-name. That spacing char '\0' is the 21th byte.
   *

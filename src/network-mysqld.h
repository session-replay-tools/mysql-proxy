...
typedef struct {
  ...
  /**
   * Called when cetus wants to send the authentication response packet to the
   * client.
   *
   * @note No known plugins implement this callback, but the default
   * implementation deals with the important case that
   *
   * the authentication response used the pre-4.1 password hash method, but the
   * client didn't.
   ...
   * into the connection, possibly in response to a result set received from a
   * server.
   *
   * This callback should not cause multiple result sets to be sent to the
   * client.
   ...
   * and reuse it later. In this case the connection state will be
   * ::ST_CLOSE_CLIENT.
   *
   ...
   *
   * @note There are no two separate callback functions for the two
   * possibilities, which probably is a deficiency.
   */
...
typedef enum { RET_SUCCESS = 10, RET_ERROR } retval_t;
...
typedef enum { ST_PROXY_OK = 0, ST_PROXY_QUIT = 1 } proxy_session_state_t;
...
struct network_mysqld_con {
  ...
  unsigned int
      is_wait_server : 1; /* first connect to backend failed, retrying */
...
typedef struct {
  ...
  short *backend_ndx_array; /* rw-only: map backend index to connections in
                               con->servers */
...
NETWORK_API int network_mysqld_con_send_ok_full(network_socket *con,
...
NETWORK_API int network_mysqld_con_send_error_full(network_socket *con,
                                                   const char *errmsg,
                                                   gsize errmsg_len,
                                                   guint errorcode,

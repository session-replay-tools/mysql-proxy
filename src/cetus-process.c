...
cetus_signal_t signals[] = {
    {cetus_signal_value(CETUS_NOACCEPT_SIGNAL),
     "SIG" cetus_value(CETUS_NOACCEPT_SIGNAL), "", cetus_signal_handler},

    {cetus_signal_value(CETUS_TERMINATE_SIGNAL),
     "SIG" cetus_value(CETUS_TERMINATE_SIGNAL), "stop", cetus_signal_handler},

    {cetus_signal_value(CETUS_SHUTDOWN_SIGNAL),
     "SIG" cetus_value(CETUS_SHUTDOWN_SIGNAL), "quit", cetus_signal_handler},

    {cetus_signal_value(CETUS_CHANGEBIN_SIGNAL),
     "SIG" cetus_value(CETUS_CHANGEBIN_SIGNAL), "", cetus_signal_handler},
...
static int create_channel(int channel[]) {
  if (socketpair(AF_UNIX, SOCK_STREAM, 0, channel) == -1) {
    g_critical("%s: socketpair() failed ", G_STRLOC);
    return -1;
  }

  g_message("%s: create channel %d:%d", G_STRLOC, channel[0], channel[1]);

  if (fcntl(channel[0], F_SETFL, O_NONBLOCK | O_RDWR) != 0) {
    g_critical("%s: nonblock failed while spawning: %s (%d)", G_STRLOC,
  ...
  if (ioctl(channel[0], FIOASYNC, &on) == -1) {
    g_critical("%s: ioctl(FIOASYNC) failed", G_STRLOC);
    cetus_close_channel(channel);
    return -1;
  }
  ...
  if (fcntl(channel[0], F_SETFD, FD_CLOEXEC) == -1) {
    g_critical("%s: fcntl(FD_CLOEXEC) failed", G_STRLOC);
    cetus_close_channel(channel);
    return -1;
  }
...
pid_t cetus_spawn_process(cetus_cycle_t *cycle, cetus_spawn_proc_pt proc,
  ...
  if (respawn >= 0) {
    ...
    if (s == CETUS_MAX_PROCESSES) {
      g_critical("%s: no more than %d processes can be spawned", G_STRLOC,
                 CETUS_MAX_PROCESSES);
  ...
  case -1:
    g_critical("%s: fork() failed while spawning \"%s\"", G_STRLOC, name);
  ...
  case 0:
    ...
    g_message("%s: after call fork, channel:%d, pid:%d", G_STRLOC, s,
              cetus_processes[s].pid);
    proc(cycle, data);
    break;
  ...
  if (s == cetus_last_process) {
    g_message("%s: cetus_last_process add,orig:%d, "
              "cetus_processes[s].parent_child_channel[0]:%d",
...
pid_t cetus_execute(cetus_cycle_t *cycle, cetus_exec_ctx_t *ctx) {
...
static void cetus_execute_proc(cetus_cycle_t *cycle, void *data) {
  ...
  if (execve(ctx->path, (char *const *)ctx->argv, (char *const *)ctx->envp) ==
      -1) {
    g_critical("%s: execve() failed while executing %s \"%s\"", G_STRLOC,
               ctx->name, ctx->path);
  }
...
static void cetus_signal_handler(int signo, siginfo_t *siginfo,
                                 void *ucontext) {
  int err;
  cetus_signal_t *sig;

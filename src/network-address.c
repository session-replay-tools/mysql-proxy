...
network_address *network_address_new() {
  ...
  g_debug("%s: create socket name %p successful for add:%p", G_STRLOC,
          addr->name, addr);
    ...
    if (name[0] == '/') {
      ...
      if (ret == 0) {
        ...
        if (errno != EPERM && errno != EACCES) {
          g_critical("%s: removing socket %s failed: %s (%d)", G_STRLOC, name,
...
static gint network_address_set_address_ip(network_address *addr,
  ...
  if (NULL == address || address[0] == '\0') {
    ...
    for (ai = first_ai; ai; ai = ai->ai_next) {
      ...
      if (family == PF_INET6) {
        memcpy(&addr->addr.ipv6, (struct sockaddr_in6 *)ai->ai_addr,
        ...
        memcpy(&addr->addr.ipv4, (struct sockaddr_in *)ai->ai_addr,
    ...
    memcpy(&(addr->addr.ipv4.sin_addr.s_addr), he->h_addr_list[0],
           he->h_length);
...
static gint network_address_set_address_un(network_address *addr,
                                           const gchar *address) {
  g_return_val_if_fail(addr, -1);
...
gint network_address_set_address(network_address *addr, const gchar *address) {
  ...
  if (NULL != port_part) {
    ...
    if (*port_part == '\0') {
      ...
      g_critical(
          "%s: IP-addr in the form [<ip>][:<port>], is '%s'. port at '%s'",
...
static const gchar *network_address_tostring_inet_ntop(network_address *addr,
                                                       gchar *dst,
                                                       gsize *dst_len,
                                                       GError **gerr) {
  const char *addr_str;
  ...
  case AF_INET:
    ...
    if (NULL == addr_str) {
      if (ENOSPC == errno) {
        g_set_error(gerr, NETWORK_ADDRESS_ERROR,
                    NETWORK_ADDRESS_ERROR_DST_TOO_SMALL,
                    "inet_ntop() failed: %s (%d)", g_strerror(errno), errno);
      } else {
        g_set_error(gerr, NETWORK_ADDRESS_ERROR, NETWORK_ADDRESS_ERROR_UNKNOWN,
                    "inet_ntop() failed: %s (%d)", g_strerror(errno), errno);
      }
      return NULL;
    }
  ...
  case AF_UNIX:
    *dst_len = g_strlcpy(dst, addr->addr.un.sun_path, *dst_len);
    ...
    if (*dst_len >= initial_dst_len) {
      ...
      g_set_error(gerr, NETWORK_ADDRESS_ERROR,
                  NETWORK_ADDRESS_ERROR_DST_TOO_SMALL, "dst too small");
      return NULL;
    }
 ...
 */
static const gchar *network_address_tostring_inet_ntoa(network_address *addr,
                                                       gchar *dst,
                                                       gsize *dst_len,
                                                       GError **gerr) {
  const char *addr_str;
...
char *network_address_tostring(network_address *addr, char *dst, gsize *dst_len,
                               GError **gerr) {
  const char *addr_str;

  if (NULL == dst) {
    g_set_error(gerr, NETWORK_ADDRESS_ERROR, NETWORK_ADDRESS_ERROR_INVALID,
                "dst is NULL");
    return NULL;
  }
  if (NULL == dst_len) {
...
gint network_address_refresh_name(network_address *addr) {
  ...
  if (addr->addr.common.sa_family == AF_INET) {
    ...
    g_string_printf(addr->name, "[%s]:%d", buf,
...
network_address *network_address_copy(network_address *dst,
                                      network_address *src) {
  if (!dst)

...
void network_mysqld_com_query_result_free(
...
int network_mysqld_proto_get_com_query_result(
  ...
  case PARSE_COM_QUERY_INIT:
    ...
    case MYSQLD_PACKET_OK: /* e.g. DELETE FROM tbl */
      ...
      if (!err) {
        ...
        g_debug("%s: server status in ok packet, got: %d", G_STRLOC,
    ...
    case MYSQLD_PACKET_EOF:
      g_critical("%s: COM_QUERY should not be (EOF), got: 0x%02x", G_STRLOC,
                 status);
      err = 1;
      break;
    default:
  ...
  case PARSE_COM_QUERY_FIELD:
    ...
    case MYSQLD_PACKET_NULL:
      g_critical("%s: COM_QUERY !=(OK|NULL|ERR), got: 0x%02x", G_STRLOC,
                 status);
    ...
    case MYSQLD_PACKET_EOF:
      ...
      if (packet->data->len == 9) {
        ...
        if (!err) {
          ...
          query->state = PARSE_COM_QUERY_RESULT;
#endif
          g_debug("%s: set query state:%d for parse.data:%p", G_STRLOC,
                  query->state, query);

          /* track the server_status of the 1st EOF packet */
          query->server_status = eof_packet->server_status;
          g_debug("%s: server status in eof packet, got: %d", G_STRLOC,
...
void network_mysqld_com_stmt_prepare_result_free(
    network_mysqld_com_stmt_prep_result_t *udata) {
  if (!udata)
    return;
...
int network_mysqld_proto_get_com_stmt_prep_result(
  ...
  if (udata->first_packet == 1) {
    ...
    case MYSQLD_PACKET_ERR:
      is_finished = 1;
      g_message(
          "%s: network_mysqld_proto_get_com_stmt_prep_rs get packet err:%d",
          G_STRLOC, status);
      udata->status = status;
      break;
    default:
...
void network_mysqld_com_init_db_result_free(
    network_mysqld_com_init_db_result_t *udata) {
  if (udata->db_name)
...
int network_mysqld_com_init_db_result_track_state(
    network_packet *packet, network_mysqld_com_init_db_result_t *udata) {
  ...
  if (packet->offset != packet->data->len) {
    ...
    network_mysqld_proto_get_gstr_len(
        packet, packet->data->len - packet->offset, udata->db_name);
  } else {
    if (udata->db_name)
      g_string_free(udata->db_name, TRUE);
    udata->db_name = NULL;
...
int network_mysqld_proto_get_com_init_db(
  ...
  /**
   * in case we have a init-db statement we track the db-change on the
  ...
  case MYSQLD_PACKET_ERR:
    ...
    if (udata->db_name && udata->db_name->len) {
      g_message(
          "%s: COM_INIT_DB failed, want db:%s, client default db still:%s",
          G_STRLOC, udata->db_name->str, con->client->default_db->str);
    } else {
      g_message("%s: COM_INIT_DB failed, client default db still:%s", G_STRLOC,
  ...
  case MYSQLD_PACKET_OK:
    ...
    if (udata->db_name && udata->db_name->len) {
      ...
      if (con->server) {
        ...
        g_debug("%s:set server default db:%s for con:%p", G_STRLOC,
                con->server->default_db->str, con);
      }

      g_string_append_len(con->client->default_db, S(udata->db_name));
      g_debug("%s: COM_INIT_DB set default db success:%s", G_STRLOC,
              con->client->default_db->str);
    } else {
      if (con->server) {
    ...
    break;
  default:
    g_critical("%s: COM_INIT_DB should be (ERR|OK), got %02x", G_STRLOC,
               status);
    return -1;
  }
 ...
 */
int network_mysqld_con_command_states_init(network_mysqld_con *con,
  ...
  case COM_STMT_PREPARE:
    ...
    con->parse.data_free =
        (GDestroyNotify)network_mysqld_com_stmt_prepare_result_free;
    break;
  case COM_INIT_DB:
    con->parse.data = network_mysqld_com_init_db_result_new();
    con->parse.data_free =
        (GDestroyNotify)network_mysqld_com_init_db_result_free;

    if (network_mysqld_com_init_db_result_track_state(packet,
                                                      con->parse.data) != 0) {
      return -1;
    }
  ...
  case COM_QUIT:
     ...
     * If b) we should not print a message at the next EV_READ event from the
     * server fd
     */
    con->com_quit_seen = 1;
  default:
 ...
 */
int network_mysqld_proto_get_query_result(network_packet *packet,
                                          network_mysqld_con *con) {
  guint8 status;
  int is_finished = 0;
  int err = 0;
  ...
  err = err || network_mysqld_proto_skip_network_header(packet);
  if (err) {
    g_message("%s: skip header error, command:%d.", G_STRLOC,
              con->parse.command);
    return -1;
  }
  ...
  case COM_RESET_CONNECTION:
    ...
    default:
      ...
      g_message(
          "%s: got a 0x%02x packet for COM_[0%02x], expected only (ERR|EOF)",
  ...
  case COM_CHANGE_USER:
    ...
    case MYSQLD_PACKET_ERR:
      ...
      if (!network_mysqld_proto_get_err_packet(packet, err_packet)) {
        g_warning("%s:error code:%d,errmsg:%s,sqlstate:%s", G_STRLOC,
                  (int)err_packet->errcode, err_packet->errmsg->str,
    ...
    default:
      g_debug_hexdump(G_STRLOC, S(packet->data));
      g_message(
          "%s: got a 0x%02x packet for COM_[0%02x], expected only (ERR|OK)",
          G_STRLOC, con->parse.command, (guint8)status);
      return -1;
    }
    break;
  ...
  case COM_FIELD_LIST:
    ...
    case MYSQLD_PACKET_OK:
      g_debug_hexdump(G_STRLOC, S(packet->data));
      g_message(
          "%s: got a 0x%02x for COM_[0%02x], expected ERR, EOF or field data",
          G_STRLOC, con->parse.command, (guint8)status);
      return -1;
    default:
      break;
  ...
  case COM_STMT_PREPARE:
    is_finished =
        network_mysqld_proto_get_com_stmt_prep_result(packet, con->parse.data);
    break;
  case COM_STMT_EXECUTE:
    /* COM_STMT_EXECUTE result packets are basically the same as COM_QUERY ones,
     * the only difference is the encoding of the actual data - fields are in
     * there, too.
     */
...
int network_mysqld_proto_get_fielddef(network_packet *packet,
  ...
  if (capabilities & CLIENT_PROTOCOL_41) {
    ...
    err = err ||
          network_mysqld_proto_get_lenenc_str(packet, &field->catalog, NULL);
    err = err || network_mysqld_proto_get_lenenc_str(packet, &field->db, NULL);
    err =
        err || network_mysqld_proto_get_lenenc_str(packet, &field->table, NULL);
    err = err ||
          network_mysqld_proto_get_lenenc_str(packet, &field->org_table, NULL);
    err =
        err || network_mysqld_proto_get_lenenc_str(packet, &field->name, NULL);
    err = err ||
          network_mysqld_proto_get_lenenc_str(packet, &field->org_name, NULL);
...
int network_mysqld_proto_get_ok_packet(network_packet *packet,
  ...
  if (field_count != 0) {
    g_critical("%s: expected the first byte to be 0, got %d", G_STRLOC,
               field_count);
    return -1;
  }
...
int network_mysqld_proto_append_ok_packet(
    GString *packet, network_mysqld_ok_packet_t *ok_packet) {
  guint32 capabilities = CLIENT_PROTOCOL_41;
  ...
  network_mysqld_proto_append_int16(packet,
                                    ok_packet->server_status); /* autocommit */
  if (capabilities & CLIENT_PROTOCOL_41) {
    network_mysqld_proto_append_int16(packet,
                                      ok_packet->warnings); /* no warnings */
  }
 ...
 */
int network_mysqld_proto_get_err_packet(
    network_packet *packet, network_mysqld_err_packet_t *err_packet) {
  ...
  if (field_count != MYSQLD_PACKET_ERR) {
    g_critical("%s: expected the first byte to be 0xff, got %d", G_STRLOC,
               field_count);
    return -1;
  }
  ...
  if (packet->offset < packet->data->len) {
    err = err || network_mysqld_proto_get_str_len(
                     packet, &errmsg, packet->data->len - packet->offset);
  }
 ...
 * @return 0 on success
 */
int network_mysqld_proto_append_err_packet(
    GString *packet, network_mysqld_err_packet_t *err_packet) {
  int errmsg_len;
 ...
 * decode a OK packet from the network packet
 */
int network_mysqld_proto_get_eof_packet(
    network_packet *packet, network_mysqld_eof_packet_t *eof_packet) {
  guint8 field_count;
  ...
  if (field_count != MYSQLD_PACKET_EOF) {
    g_critical("%s: expected the first byte to be 0xfe, got %d", G_STRLOC,
               field_count);
    return -1;
  }
...
void network_mysqld_auth_challenge_set_challenge(
    network_mysqld_auth_challenge *shake) {
  guint i;
...
int network_mysqld_proto_get_auth_challenge(
    network_packet *packet, network_mysqld_auth_challenge *shake) {
  int maj, min, patch;
  ...
  err = err ||
        network_mysqld_proto_get_string(packet, &shake->server_version_str);
  err =
      err || (NULL ==
              shake->server_version_str); /* the server-version has to be set */
  ...
  /* capabilities is extended in 5.5.x to carry 32bits to announce
   * CLIENT_PLUGIN_AUTH */
  ...
  if (!err) {
    ...
    if (shake->capabilities & CLIENT_PLUGIN_AUTH) {
      ...
      err = err || network_mysqld_proto_skip(
                       packet, 12 - MIN(12, auth_plugin_data2_len));
      if (!err) {
         ...
         */
        if (0 !=
            network_mysqld_proto_get_gstr(packet, shake->auth_plugin_name)) {
          err = err || network_mysqld_proto_get_gstr_len(
      ...
      err = err ||
            network_mysqld_proto_get_str_len(packet, &auth_plugin_data_2, 12);
      err = err || network_mysqld_proto_skip(packet, 1);
    }
  }
  ...
  if (!err) {
    ...
    if (3 !=
        sscanf(shake->server_version_str, "%d.%d.%d%*s", &maj, &min, &patch)) {
...
int network_mysqld_proto_append_auth_challenge(
    GString *packet, network_mysqld_auth_challenge *shake) {
  guint i;
  ...
  if (shake->server_version_str) {
    ...
    g_string_append_printf(
        packet, "%d.%02d.%02d", shake->server_version / 10000,
...
int network_mysqld_proto_get_auth_response(network_packet *packet,
  ...
  if (l_cap & CLIENT_PROTOCOL_41) {
    ...
    if (err == 0 && (auth->client_capabilities & CLIENT_SSL) &&
    ...
    err = err || network_mysqld_proto_get_int8(packet, &len);
    err = err || network_mysqld_proto_get_gstr_len(packet, len,
                                                   auth->auth_plugin_data);
    ...
    if ((auth->server_capabilities & CLIENT_PLUGIN_AUTH) &&
      ...
      err =
          err || network_mysqld_proto_get_gstr(packet, auth->auth_plugin_name);
    }
  } else {
 ...
 */
int network_mysqld_proto_append_auth_response(
    GString *packet, network_mysqld_auth_response *auth) {
  if (!(auth->client_capabilities & CLIENT_PROTOCOL_41)) {
    ...
    network_mysqld_proto_append_int24(
        packet, auth->max_packet_size); /* max-allowed-packet */
    ...
    if (auth->auth_plugin_data->len) {
      g_string_append_len(packet,
                          S(auth->auth_plugin_data)); /* no trailing \0 */
    }
  } else {
...
int network_mysqld_proto_change_stmt_id_from_ok_packet(network_packet *packet,
  ...
  if (0x00 != packet_type) {
    g_debug("%s: expected the first byte to be %02x, got %02x", G_STRLOC, 0x00,
            packet_type);
    return -1;
  }
...
int network_mysqld_proto_append_query_packet(GString *packet,
                                             const char *query) {
...
int mysqld_proto_append_change_user_packet(
    GString *packet, mysqld_change_user_packet_t *chuser) {
  ...
  if (!chuser->auth_plugin_data) {
    ...
    network_mysqld_proto_password_scramble(
        auth_response, S(chuser->auth_plugin_data), S(chuser->hashed_pwd));

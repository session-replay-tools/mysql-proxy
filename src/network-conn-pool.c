...
network_connection_pool *network_connection_pool_new(void) {
  ...
  pool->users =
      g_hash_table_new_full(g_hash_table_string_hash, g_hash_table_string_equal,
                            g_hash_table_string_free, g_queue_free_all);
 ...
 */
static gboolean find_idle_conns(gpointer UNUSED_PARAM(_key), gpointer _val,
                                gpointer _user_data) {
...
GQueue *network_connection_pool_get_conns(network_connection_pool *pool,
  ...
  if (username && username->len > 0) {
     ...
     */
    g_debug("%s: get user-specific idling connection for '%s' -> %p", G_STRLOC,
            username->str, conns);
  ...
  conns = g_hash_table_find(pool->users, find_idle_conns,
...
network_socket *network_connection_pool_get(network_connection_pool *pool,
  ...
  if (conns) {
    if (conns->length > 0) {
      ...
      g_debug("%s: (get) entry for user '%s' -> %p", G_STRLOC,
              username ? username->str : "", entry);
    } else {
      g_debug("%s: conns length is zero for user '%s'", G_STRLOC,
              username ? username->str : "");
    }
  } else {
    g_debug("%s: conns is null for user '%s'", G_STRLOC,
            username ? username->str : "");
  }
  ...
  g_debug("%s: recv queue length:%d, sock:%p", G_STRLOC,
          sock->recv_queue->chunks->length, sock);
  ...
  if (sock->is_in_sess_context) {
    g_message("%s: conn is in sess context for user:'%s'", G_STRLOC,
              username ? username->str : "");
  }

  pool->cur_idle_connections--;
  g_debug("%s: cur_idle_connections sub:%d for sock:%p", G_STRLOC,
          pool->cur_idle_connections, sock);
...
network_connection_pool_add(network_connection_pool *pool,
                            network_socket *sock) {
  ...
  g_debug("%s: (add) adding socket to pool for user '%s' -> %p", G_STRLOC,
          sock->response->username->str, sock);

  GQueue *conns = NULL;
  if (NULL ==
      (conns = g_hash_table_lookup(pool->users, sock->response->username))) {
    conns = g_queue_new();
    g_hash_table_insert(pool->users, g_string_dup(sock->response->username),
                        conns);
  }
  ...
  pool->cur_idle_connections++;
  g_debug("%s: add cur_idle_connections:%d for sock:%p", G_STRLOC,
          pool->cur_idle_connections, sock);
 ...
 */
void network_connection_pool_remove(network_connection_pool_entry *entry) {
...
int network_connection_pool_total_conns_count(network_connection_pool *pool) {
  ...
  if (pool->cur_idle_connections != total) {
    g_warning("%s: pool cur idle connections stat error:%d, total:%d", G_STRLOC,

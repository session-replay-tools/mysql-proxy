...
int chassis_frontend_init_glib() {
  ...
  check_str = glib_check_version(GLIB_MAJOR_VERSION, GLIB_MINOR_VERSION,
                                 GLIB_MICRO_VERSION);
...
int chassis_frontend_load_plugins(GPtrArray *plugins, const gchar *plugin_dir,
                                  gchar **plugin_names) {
  int i;
...
#define SHARED_LIBRARY_SUFFIX G_MODULE_SUFFIX
    ...
    plugin_filename = g_strdup_printf("%s%c%s%s.%s", plugin_dir,
                                      G_DIR_SEPARATOR, G_MODULE_PREFIX,
                                      plugin_names[i], SHARED_LIBRARY_SUFFIX);
...
int chassis_frontend_init_plugins(GPtrArray *plugins, chassis_options_t *opts,
                                  chassis_config_t *config_manager, int *argc_p,
                                  char ***argv_p, GKeyFile *keyfile,
    ...
    if (NULL != (config_entries = chassis_plugin_get_options(p))) {
      ...
      if (keyfile) {
        if (FALSE == chassis_keyfile_to_options_with_error(
                         keyfile, keyfile_section_name, config_entries, gerr)) {
          return -1;
        }
      }
...
GKeyFile *chassis_frontend_open_config_file(const char *filename,
  ...
  if (chassis_filemode_check_full(filename, CHASSIS_FILEMODE_SECURE_MASK,
                                  gerr) != 0) {
    return NULL;
  }
  ...
  if (FALSE ==
      g_key_file_load_from_file(keyfile, filename, G_KEY_FILE_NONE, gerr)) {
...
int chassis_options_set_cmdline_only_options(chassis_options_t *opts,
  ...
  chassis_options_add(opts, "version", 'V', 0, OPTION_ARG_NONE, print_version,
                      "Show version", NULL, NULL, NULL, 0);

  chassis_options_add(opts, "defaults-file", 0, 0, OPTION_ARG_STRING,
                      config_file, "configuration file", "<file>", NULL, NULL,
                      0);
...
int chassis_frontend_write_pidfile(const char *pid_file, GError **gerr) {
  ...
  if (-1 == (fd = open(pid_file, O_WRONLY | O_TRUNC | O_CREAT, 0600))) {
    g_set_error(gerr, G_FILE_ERROR, g_file_error_from_errno(errno),
                "%s: open(%s) failed: %s", G_STRLOC, pid_file,
                g_strerror(errno));
  ...
  if (write(fd, pid_str, strlen(pid_str)) < 0) {
    g_set_error(gerr, G_FILE_ERROR, g_file_error_from_errno(errno),
                "%s: write(%s) of %s failed: %s", G_STRLOC, pid_file, pid_str,
                g_strerror(errno));
    ret = -1;
  }
  g_free(pid_str);

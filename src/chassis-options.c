 ...
 * GOptionEntry
 */
int chassis_option_set(chassis_option_t *opt, const char *long_name,
                       gchar short_name, gint flags, enum option_type arg,
                       gpointer arg_data, const char *description,
                       const char *arg_desc, chas_opt_assign_hook assign_hook,
                       chas_opt_show_hook show_hook, gint opt_property) {
...
int chassis_options_add(chassis_options_t *opts, const char *long_name,
                        gchar short_name, int flags, enum option_type arg,
                        gpointer arg_data, const char *description,
                        const char *arg_desc, chas_opt_assign_hook assign_hook,
                        chas_opt_show_hook show_hook, gint opt_property) {
  chassis_option_t *opt = chassis_option_new();
  if (0 != chassis_option_set(opt, long_name, short_name, flags, arg, arg_data,
                              description, arg_desc, assign_hook, show_hook,
                              opt_property) ||
...
static struct opt_change *get_change(chassis_options_t *context,
                                     enum option_type arg_type,
                                     gpointer arg_data) {
  GList *list;
...
static void add_pending_null(chassis_options_t *context, gchar **ptr,
                             gchar *value) {
...
static gboolean parse_arg(chassis_options_t *context, chassis_option_t *entry,
                          const gchar *value, const gchar *option_name,
                          GError **error) {
...
static gboolean parse_short_option(chassis_options_t *context, gint idx,
                                   gint *new_idx, gchar arg, gint *argc,
...
static gboolean parse_long_option(chassis_options_t *context, gint *idx,
                                  gchar *arg, gboolean aliased, gint *argc,
                                  gchar ***argv, GError **error,
                                  gboolean *parsed) {
  GList *l;
  for (l = context->options; l; l = l->next) {
      ...
      if (strncmp(arg, entry->long_name, len) == 0 &&
        ...
        if (arg[len] == '=') {
          ...
          if (!OPTIONAL_ARG(entry)) {
            ...
            if ((*argv)[*idx + 1][0] == '-') {
              gboolean retval =
                  parse_arg(context, entry, NULL, option_name, error);
              *parsed = TRUE;
              g_free(option_name);
              return retval;
            } else {
...
static gboolean parse_remaining_arg(chassis_options_t *context, gint *idx,
...
static void free_pending_nulls(chassis_options_t *context,
...
gboolean chassis_options_parse_cmdline(chassis_options_t *context, int *argc,
  ...
  for (i = 1; i < *argc; i++) {
    ...
    if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing) {
      if ((*argv)[i][1] == '-') {
        ...
        if (!parse_long_option(context, &i, arg, FALSE, argc, argv, error,
                               &parsed))
          goto fail;
        ...
        gboolean *nulled_out = g_newa(gboolean, arg_length);
        ...
        for (j = 0; j < arg_length; j++) {
          if (context->help_enabled &&
            ...
            print_help(context);
          parsed = FALSE;
          if (!parse_short_option(context, i, &new_i, arg[j], argc, argv, error,
                                  &parsed))
            goto fail;

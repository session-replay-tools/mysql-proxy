...
int chassis_check_version(const char *lib_version, const char *hdr_version) {
  ...
  if (2 !=
      (scanned_fields = sscanf(lib_version, "%d.%d", &lib_maj, &lib_min))) {
    g_critical("%s: library version %s failed to parse: %d", G_STRLOC,
               lib_version, scanned_fields);
    return -1;
  }
  if (2 !=
      (scanned_fields = sscanf(hdr_version, "%d.%d", &hdr_maj, &hdr_min))) {
    g_critical("%s: header version %s failed to parse: %d", G_STRLOC,
               hdr_version, scanned_fields);
    return -1;
  }
...
chassis *chassis_new() {
  ...
  if (0 != chassis_check_version(event_get_version(), _EVENT_VERSION)) {
    g_critical(
        "%s: chassis is built against libevent %s, but now runs against %s",
...
void chassis_free(chassis *chas) {
  ...
  /* libevent < 1.3e doesn't cleanup its own fds from the event-queue in
   * signal_init() calling event_base_free() would cause a assert() on shutdown
   */
  ...
  if (chas->user) {
    ...
    if (chas->log->log_filename) {
      ...
      if (-1 == chown(chas->log->log_filename, user_info->pw_uid,
                      user_info->pw_gid)) {
        g_critical("%s: chown(%s) failed: %s", G_STRLOC,
                   chas->log->log_filename, g_strerror(errno));
    ...
    g_debug("now running as user: %s (%d/%d)", chas->user, user_info->pw_uid,
            user_info->pw_gid);
  }

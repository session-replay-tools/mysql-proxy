...
chassis_plugin *chassis_plugin_load(const gchar *name) {
  ...
  if (!g_module_symbol(p->module, "plugin_init", (gpointer)&plugin_init)) {
    g_critical("module '%s' doesn't have a init-function: %s", name,
               g_module_error());
    chassis_plugin_free(p);
    return NULL;
  }
  ...
  if (p->magic != CHASSIS_PLUGIN_MAGIC) {
    g_critical("'%s' doesn't match the current interface (plugin is %lx, "
               "chassis is %lx)",
  ...
  if (!p->version) {
    g_critical(
        "plugin '%s' doesn't set a version num, refusing to load this plugin",
        name);
    chassis_plugin_free(p);
    return NULL;
  }

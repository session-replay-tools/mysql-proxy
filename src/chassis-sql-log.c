...
static guint rfifo_flush(struct sql_log_mgr *mgr) {
  ...
  guint s1 = pwrite(fd, fifo->buffer + (fifo->out & (fifo->size - 1)), l,
                    (off_t)mgr->sql_log_cursize);
...
struct sql_log_mgr *sql_log_alloc() {
  struct sql_log_mgr *mgr =
      (struct sql_log_mgr *)g_malloc0(sizeof(struct sql_log_mgr));
...
static void sql_log_check_filenum(struct sql_log_mgr *mgr, gchar *filename) {
  if (!mgr || !filename) {
    g_warning(
        "struct mgr or filename is NULL when call sql_log_check_filenum()");
    return;
  }
  ...
  if (!mgr->sql_log_filelist) {
    ...
    GList *pos = g_queue_find_custom(mgr->sql_log_filelist, filename,
                                     (GCompareFunc)strcmp);
    if (pos) {
...
static void sql_log_check_rotate(struct sql_log_mgr *mgr) {
  if (!mgr)
    return;
  if (mgr->sql_log_maxsize == 0)
    return;
  if (mgr->sql_log_cursize < ((gulong)mgr->sql_log_maxsize) * MEGABYTES)
    return;
  ...
  gchar *rotate_filename =
      g_strdup_printf("%s/%s-%d-%04d%02d%02d%02d%02d%02d.%s", mgr->sql_log_path,
                      mgr->sql_log_prefix, cetus_pid, cur_tm.tm_year + 1900,
...
gpointer sql_log_mainloop(gpointer user_data) {
  ...
  if (!mgr->sql_log_fp) {
    g_critical("sql log thread exit, because fopen(%s) failed",
               mgr->sql_log_fullname);
    mgr->sql_log_action = SQL_LOG_STOP;
    return NULL;
  }
...
void cetus_sql_log_start_thread_once(struct sql_log_mgr *mgr) {
  ...
  if (mgr->sql_log_fullname == NULL) {
    mgr->sql_log_fullname =
        g_strdup_printf("%s/%s-%d.%s", mgr->sql_log_path, mgr->sql_log_prefix,
                        cetus_pid, SQL_LOG_DEF_SUFFIX);
  }
...
void log_sql_client(network_mysqld_con *con) {
  ...
  if (mgr->sql_log_switch == OFF || !(mgr->sql_log_mode & CLIENT) ||
      (mgr->sql_log_action != SQL_LOG_START)) {
    return;
  }
  ...
  g_string_append_printf(message,
                         ": #client# C_ip:%s C_db:%s C_usr:%s C_tx:%s "
                         "C_retry:%d C_id:%u type:%s %s\n",
                         ...
                         con->orig_sql != NULL ? con->orig_sql->str
                                               : ""); // sql
...
void log_sql_backend(network_mysqld_con *con, injection *inj) {
  ...
  if (mgr->sql_log_switch == OFF || !(mgr->sql_log_mode & BACKEND) ||
      (mgr->sql_log_action != SQL_LOG_START)) {
    return;
  }
  gdouble latency_ms =
      (inj->ts_read_query_result_last - inj->ts_read_query) / 1000.0;
  GString *message = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
  get_current_time_str(message);
  g_string_append_printf(
      message,
      ": #backend-rw# C_ip:%s C_db:%s C_usr:%s C_tx:%s C_id:%u S_ip:%s S_db:%s "
      "S_usr:%s S_id:%u "
      ...
      inj->id, inj->bytes, inj->rows, latency_ms,
...
void log_sql_connect(network_mysqld_con *con, gchar *errmsg) {
  ...
  if (mgr->sql_log_switch == OFF || !(mgr->sql_log_mode & CONNECT) ||
      (mgr->sql_log_action != SQL_LOG_START)) {
    return;
  }
  GString *message = g_string_sized_new(sizeof("2004-01-01T00:00:00.000Z"));
  ...
  g_string_append_printf(
      message,
      ": #connect# %s@%s Connect Cetus %s msg:%s, C_id:%u C_db:%s C_charset:%u "
      "C_auth_plugin:%s C_ssl:%s C_cap:%x S_cap:%x\n",
      con->client->response->username->str, // C_usr
      con->client->src->name->str,          // C_ip
      errmsg == NULL ? "success" : "failed", errmsg == NULL ? "" : errmsg,
      con->client->challenge->thread_id,    // C_id

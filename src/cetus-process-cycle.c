...
static int open_plugins(cetus_cycle_t *cycle) {
  int i;
  ...
  for (i = 0; i < cycle->modules->len; i++) {
    ...
    if (strcmp(p->name, "admin") == 0) {
      ...
      if (0 != p->apply_config(cycle, p->config)) {
        g_critical("%s: applying config of plugin %s failed", G_STRLOC,
                   p->name);
        return -1;
      }
    }
...
cetus_pid_t cetus_exec_new_binary(cetus_cycle_t *cycle, char **argv) {
  ...
  while (!chassis_is_shutdown()) {
    if (!conf->options_update_flag) {
      ...
      case ASYNCHRONOUS_UPDATE_OR_DELETE_USER_PASSWORD:
        ...
        chassis_config_mysql_write_object(conf, object, "users",
                                          conf->user_data);
        g_free(conf->user_data);
        conf->user_data = NULL;
        g_message("%s: ASYNCHRONOUS_UPDATE_OR_DELETE_USER_PASSWORD visited",
                  G_STRLOC);
        break;
      ...
      case ASYNCHRONOUS_UPDATE_BACKENDS:
        ...
        chassis_config_set_remote_backends(conf, conf->key, conf->value,
                                           conf->reserve1, conf->reserve2);
        break;
      default:
        break;
      }
...
static void cetus_remote_config_start_thread(chassis *chas) {
  GThread *new_thread;
#if !GLIB_CHECK_VERSION(2, 32, 0)
  GError *error = NULL;
  new_thread =
      g_thread_create(retrieve_remote_config_mainloop, chas, TRUE, &error);
  if (new_thread == NULL && error != NULL) {
    g_critical("%s:Create thread error: %s", G_STRLOC, error->message);
    g_clear_error(&error);
  }
#else
  new_thread = g_thread_new("remote-config-thread",
                            retrieve_remote_config_mainloop, chas);
  if (new_thread == NULL) {
...
void cetus_master_process_cycle(cetus_cycle_t *cycle) {
  ...
  for (;;) {
          ...
          if (diff > 1) {
            ...
            if (cycle->child_instant_exit_times >= cetus_last_process) {
              ...
              g_message("%s: set cetus_terminate is true:%d, workers:%d",
                        G_STRLOC, cycle->child_instant_exit_times,
                        cetus_last_process);
            }
          }
        }
    ...
    if (cetus_terminate) {
      if (live && try_cnt >= 10) {
        ...
        if (cycle->worker_processes) {
          cetus_signal_worker_processes(
              cycle, cetus_signal_value(CETUS_TERMINATE_SIGNAL));
        }
      }
      continue;
    }
    ...
    if (cetus_quit) {
      if (cycle->worker_processes) {
        cetus_signal_worker_processes(
            cycle, cetus_signal_value(CETUS_SHUTDOWN_SIGNAL));
      }
      live = 0;
      continue;
    }
    ...
    if (cetus_noaccept) {
      ...
      if (cycle->worker_processes) {
        cetus_signal_worker_processes(
            cycle, cetus_signal_value(CETUS_NOACCEPT_SIGNAL));
      }
    }
    ...
    if (cetus_change_binary) {
      ...
      if (cycle->active_worker_processes > 0) {
        g_message("%s: changing binary when active processes:%d", G_STRLOC,
                  cycle->active_worker_processes);
      } else {
        cetus_quit = 1;
      }
...
static void cetus_start_worker_processes(cetus_cycle_t *cycle, int n,
                                         int type) {
  int i;
  cetus_channel_t ch;
  ...
  for (i = 0; i < n; i++) {
    ...
    cetus_spawn_process(cycle, cetus_worker_process_cycle, (void *)(intptr_t)i,
                        "worker process", type);
    ...
    g_debug("%s: call cetus_pass_open_channel, cetus_process_slot:%d, pid:%d, "
            "fd:%d",
...
static void cetus_pass_open_channel(cetus_cycle_t *cycle, cetus_channel_t *ch) {
  ...
  g_debug("%s: call cetus_pass_open_channel, cetus_last_process:%d", G_STRLOC,
          cetus_last_process);

  for (i = 0; i < cetus_last_process; i++) {

    g_debug("%s: i:%d,pid:%d,fd:%d ", G_STRLOC, i, cetus_processes[i].pid,
            cetus_processes[i].parent_child_channel[0]);

    if (i == cetus_process_slot || cetus_processes[i].pid == -1 ||
        cetus_processes[i].parent_child_channel[0] == -1) {
      continue;
    }

    g_message("%s: pass channel s:%i pid:%d fd:%d to s:%i pid:%d fd:%d, ev "
              "base:%p, ev:%p",
              G_STRLOC, ch->basics.slot, ch->basics.pid, ch->basics.fd, i,
              cetus_processes[i].pid,
              cetus_processes[i].parent_child_channel[0], cycle->event_base,
              &cetus_channel_event);

    /* TODO: AGAIN */
    cetus_write_channel(cetus_processes[i].parent_child_channel[0], ch,
                        sizeof(cetus_channel_mininum_t));
  }
}
...
static void cetus_signal_worker_processes(cetus_cycle_t *cycle, int signo) {
  ...
  for (i = 0; i < cetus_last_process; i++) {
    ...
    g_debug("%s: child: %i %d e:%d t:%d d:%d r:%d j:%d", G_STRLOC, i,
            cetus_processes[i].pid, cetus_processes[i].exiting,
            cetus_processes[i].exited, cetus_processes[i].detached,
            cetus_processes[i].respawn, cetus_processes[i].just_spawn);
    ...
    if (ch.basics.command) {
      ...
      if (cetus_write_channel(cetus_processes[i].parent_child_channel[0], &ch,
                              sizeof(cetus_channel_mininum_t)) ==
    ...
    if (kill(cetus_processes[i].pid, signo) == -1) {
      err = errno;
      g_critical("%s: kill (%d, %d) failed", G_STRLOC, cetus_processes[i].pid,
                 signo);
...
static unsigned int cetus_reap_children(cetus_cycle_t *cycle) {
  ...
  for (i = 0; i < cetus_last_process; i++) {
    ...
    g_message("%s: child: %i %d e:%d t:%d d:%d r:%d j:%d", G_STRLOC, i,
              cetus_processes[i].pid, cetus_processes[i].exiting,
              cetus_processes[i].exited, cetus_processes[i].detached,
              cetus_processes[i].respawn, cetus_processes[i].just_spawn);
    ...
    if (cetus_processes[i].exited) {
      ...
      if (!cetus_processes[i].detached || cycle->config_changed) {
        ...
        for (n = 0; n < cetus_last_process; n++) {
          if (cetus_processes[n].exited || cetus_processes[n].pid == -1 ||
          ...
          /* TODO: AGAIN */
          cetus_write_channel(cetus_processes[n].parent_child_channel[0], &ch,
                              sizeof(cetus_channel_mininum_t));
        }
      }

      if (cetus_processes[i].respawn && !cetus_processes[i].exiting &&
          !cetus_terminate && !cetus_quit) {

        usleep(1000 * 1000);
        g_debug("%s: before call cetus_spawn_process", G_STRLOC);
        if (cetus_spawn_process(
                cycle, cetus_processes[i].proc, cetus_processes[i].data,
                cetus_processes[i].name, i) == CETUS_INVALID_PID) {
          g_critical("%s: could not respawn %s", G_STRLOC,
                     cetus_processes[i].name);
          continue;
        }
        ...
        ch.basics.slot = cetus_process_slot;
        ch.basics.fd =
            cetus_processes[cetus_process_slot].parent_child_channel[0];

        g_debug("%s: call cetus_pass_open_channel, slot:%d", G_STRLOC,
                cetus_process_slot);
        cetus_pass_open_channel(cycle, &ch);
      ...
      if (i == cetus_last_process - 1) {
        g_message("%s: cetus_last_process sub,orig:%d", G_STRLOC,
                  cetus_last_process);
        cetus_last_process--;
...
static void cetus_worker_process_cycle(cetus_cycle_t *cycle, void *data) {
  ...
  if (cycle->cpus > 0) {
    ...
    if (sched_setaffinity(0, sizeof(cpu_set), &cpu_set) < 0) {
      g_critical("%s: failed to pin to cpu:%s, cpu index:%d", G_STRLOC,
                 strerror(errno), cpu_ndx);
    }
  }
#endif
...
static void cetus_worker_process_init(cetus_cycle_t *cycle, int worker) {
  ...
  for (n = 0; n < cetus_last_process; n++) {
    ...
    g_debug("%s: close() channel one fd:%d, n:%d", G_STRLOC,
            cetus_processes[n].parent_child_channel[1], n);

    if (close(cetus_processes[n].parent_child_channel[1]) == -1) {
      g_critical("%s: close() channel failed, err:%s", G_STRLOC,
                 strerror(errno));
    }
  }

  g_debug("%s: close() channel zero fd:%d, n:%d", G_STRLOC,
          cetus_processes[cetus_process_slot].parent_child_channel[0],
          cetus_process_slot);
  if (close(cetus_processes[cetus_process_slot].parent_child_channel[0]) ==
      -1) {
    g_critical("%s: close() channel failed, strerr:%s", G_STRLOC,
               strerror(errno));
  }

  g_debug("%s: channel fd for recving:%d, n:%d", G_STRLOC,
  ...
  event_set(&cetus_channel_event, cetus_channel, EV_READ | EV_PERSIST,
            cetus_channel_handler, cycle);
...
static cetus_channel_t *retrieve_admin_resp(network_mysqld_con *con) {
  GList *chunk;
  g_debug("%s:call retrieve_admin_resp", G_STRLOC);
  int total = sizeof(cetus_channel_t);
  int resp_len = 0;
  for (chunk = con->client->send_queue->chunks->head; chunk;
       chunk = chunk->next) {
...
{
  if (cycle->worker_processes) {
    g_debug("%s:call send_admin_resp, cetus_process_slot:%d", G_STRLOC,
            cetus_process_slot);
    ...
    g_debug("%s:send resp to admin, cetus_process_slot:%d", G_STRLOC,
            cetus_process_slot);
    g_debug("%s: pass sql resp channel s:%i pid:%d to:%d, fd:%d", G_STRLOC,
            ch->basics.slot, ch->basics.pid,
            cetus_processes[cetus_process_slot].pid,
            cetus_processes[cetus_process_slot].parent_child_channel[1]);

    /* TODO: AGAIN */
    cetus_write_channel(
        cetus_processes[cetus_process_slot].parent_child_channel[1], ch,
        sizeof(*ch) + ch->admin_sql_resp_len);
    g_debug("%s:cetus_write_channel send:%d", G_STRLOC,
            (int)(sizeof(*ch) + ch->admin_sql_resp_len));
    g_free(ch);
...
static void process_admin_sql(cetus_cycle_t *cycle, cetus_channel_t *ch) {
  ...
  if (cycle->admin_plugin) {
    ...
    NETWORK_MYSQLD_PLUGIN_FUNC(func) = plugin->con_exectute_sql;
    network_socket_retval_t retval = (*func)(cycle, con);
...
static void cetus_channel_handler(int fd, short events, void *user_data) {
  ...
  g_debug("%s: channel handler, cetus_last_process:%d", G_STRLOC,
          cetus_last_process);
  ...
  do {
    ...
    g_debug("%s: after cetus_read_channel channel, fd:%d, ret:%d", G_STRLOC, fd,
            ret);
    ...
    case CETUS_CMD_OPEN_CHANNEL:
      ...
      g_debug("%s: get channel s:%i pid:%d fd:%d", G_STRLOC, ch.basics.slot,
              ch.basics.pid, ch.basics.fd);
    ...
    case CETUS_CMD_CLOSE_CHANNEL:
      ...
      g_debug("%s: close channel s:%i pid:%d our:%d fd:%d", G_STRLOC,
              ch.basics.slot, ch.basics.pid,
              cetus_processes[ch.basics.slot].pid,
              cetus_processes[ch.basics.slot].parent_child_channel[0]);

      if (close(cetus_processes[ch.basics.slot].parent_child_channel[0]) ==
          -1) {

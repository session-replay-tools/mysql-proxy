...
const struct {
  ...
  int win_evtype;
    ...
    {/* syslog levels are different to the glib ones */
...
static gboolean chassis_log_rotate_reopen(chassis_log *log, gpointer userdata,
                                          GError **gerr);
 ...
 */
void chassis_log_set_rotate_func(chassis_log *log,
...
static void chassis_log_func_locked(const gchar G_GNUC_UNUSED *log_domain,
   ...
   * 30 seconds ago.
   */
  if (log->is_rotated || !is_duplicate || log->last_msg_count > 100 ||
    ...
    if (log->last_msg_count) {
      ...
      g_string_append_printf(log->log_ts_str,
...
void chassis_log_set_logrotate(chassis_log *log) { log->rotate_logs = TRUE; }

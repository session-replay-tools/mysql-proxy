...
gboolean chassis_keyfile_to_options_with_error(GKeyFile *keyfile,
                                               const gchar *ini_group_name,
                                               GList *config_entries,
                                               GError **_gerr) {
  GError *gerr = NULL;
  gboolean ret = TRUE;
  int j;
  ...
  for (l = config_entries; l; l = l->next) {
    ...
    case OPTION_ARG_STRING_ARRAY:
      ...
      arg_string_array = g_key_file_get_string_list(
          keyfile, ini_group_name, entry->long_name, &len, &gerr);
      if (!gerr) {
    ...
    case OPTION_ARG_INT64:
      arg_int64 = g_key_file_get_int64(keyfile, ini_group_name,
                                       entry->long_name, &gerr);
      if (!gerr) {
    ...
    case OPTION_ARG_DOUBLE:
      ...
      break;
    default:
      g_error("%s: (keyfile) the option %d can't be handled", G_STRLOC,
              entry->arg);
      break;
    }

...
void cetus_compress_end(z_stream *strm) { (void)deflateEnd(strm); }

int cetus_compress(z_stream *strm, GString *dst, char *src, int src_len,
                   int end) {
  int flush;
...
static void cetus_uncompress_end(z_stream *strm) { (void)inflateEnd(strm); }

int cetus_uncompress(GString *uncompressed_packet, unsigned char *src,
                     int len) {
  int ret;
  z_stream strm;
  unsigned char out[CHUNK];

...
#define GET_COM_NAME(com_type)                                                 \
  ((((gushort)com_type) <= 31) ? com_command_name[((gushort)com_type)].com_str \
                               : "UNKNOWN TYPE")
...
typedef enum { OFF, ON, REALTIME } SQL_LOG_SWITCH;
...
typedef enum { SQL_LOG_UNKNOWN, SQL_LOG_START, SQL_LOG_STOP } SQL_LOG_ACTION;

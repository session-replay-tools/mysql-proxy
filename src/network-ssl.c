...
static void network_ssl_info_callback(const SSL *s, int where, int ret)
{
  ...
  if (where & SSL_CB_LOOP) {
    ...
    g_warning("SSL3 alert %s:%s:%s", str, SSL_alert_type_string_long(ret),
              SSL_alert_desc_string_long(ret));
  } else if (where & SSL_CB_EXIT) {
    if (ret == 0)
      g_warning("%s:failed in %s", str, SSL_state_string_long(s));
    else if (ret < 0) {
      g_warning("%s:error in %s", str, SSL_state_string_long(s));
    }
  }
}
...
static gboolean network_ssl_create_context(char* conf_dir)
{
  ...
  SSL_CTX_set_read_ahead(g_ssl_context,
                         1); /* SSL_read clear data on the wire */
...
network_socket_retval_t network_ssl_write(network_socket *sock,
  ...
  network_queue *send_queue =
      sock->do_compress ? sock->send_queue_compressed : sock->send_queue;
...
gboolean network_ssl_decrypt_packet(network_socket* sock)
{
  ...
  while (TRUE) {
    ...
    while (TRUE) {
      ...
      network_queue_append(sock->recv_queue_decrypted_raw,
                           g_string_new_len(buf, len));
    }
  }
}

...
static void pwd_pair_set_pwd(struct pwd_pair_t *pwd, const char *new_pass,
                             enum cetus_pwd_type t) {
  switch (t) {
...
static gboolean pwd_pair_same_pwd(struct pwd_pair_t *pwd, const char *new_pass,
                                  enum cetus_pwd_type t) {
  switch (t) {
  case CETUS_CLIENT_PWD:
...
static void cetus_users_set_records(cetus_users_t *users,
                                    GHashTable *new_records) {
  if (users->records) {
    g_hash_table_destroy(users->records);
  }
...
gboolean cetus_users_parse_json(cetus_users_t *users, char *buffer) {
  ...
  GHashTable *user_records = g_hash_table_new_full(
      g_str_hash, g_str_equal, g_free, (GDestroyNotify)pwd_pair_free);
  ...
  for (; user_node; user_node = user_node->next) {
    ...
    if (!client_pwd && !server_pwd) {
      g_critical(G_STRLOC
                 ":user conf error, at least one of client/server is needed");
      goto out;
    }
    if (client_pwd && !server_pwd)
    ...
    g_hash_table_insert(
        user_records, g_strdup(username->valuestring),
...
gboolean cetus_users_update_record(cetus_users_t *users, const char *user,
  ...
  if (pwd) {
    ...
    g_hash_table_insert(users->records, g_strdup(user),
                        pwd_pair_new(pass, pass));
  }
...
gboolean cetus_users_write_json(cetus_users_t *users) {
  ...
  while (
      g_hash_table_iter_next(&iter, (gpointer *)&username, (gpointer *)&pwd)) {
...
cetus_users_authenticate_client(cetus_users_t *users,
  ...
  if (pwd->client) {
    ...
    network_mysqld_proto_password_hash(sha1_pwd, pwd->client,
                                       strlen(pwd->client));
    GString *expected_response = g_string_new(NULL);
    network_mysqld_proto_password_scramble(
        expected_response, S(challenge->auth_plugin_data), S(sha1_pwd));
...
void cetus_users_get_hashed_server_pwd(cetus_users_t *users,
  ...
  if (pwd->server) {
    network_mysqld_proto_password_hash(sha1_pwd, pwd->server,
                                       strlen(pwd->server));
  }
}
